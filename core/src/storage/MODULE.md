...
>
> **Rollback log:**
> - **2026-07-02 (D-141 cascade — sp_id column + Ph-1 dashboard helpers)** — `DeliveryItemTable` gains `sp_id: int | None` column with index `ix_di_customer_sp` on `(customer_id, sp_id)`. sp_id is SP's Deliverables list row Id (auto-counter PK); populated at `import_deliverable_tracker_task` time via SP READ by natural key `(project_model, item_no)` after `create_delivery_item`. Nullable (defensive against SP READ transient failures + rows imported before this cascade). New helpers: `get_by_customer_and_sp_id(customer_id, sp_id) -> DeliveryItemBase | None` (dashboard `/docs/<customer_id>/<sp_id>` lookup key per architect lock 2026-07-01), `list_documents_for_item_display(delivery_item_id) -> list[tuple[filename, doc_type, ingested_at]]` (classified-only, ordered by ingested_at DESC; Ph-1 minimal doc-table row source). `PostgresStorage` sync wrappers added for both. `configure_engine` DB URL resolver accepts BOTH `HILDA_STORAGE_DB_URL` (canonical) AND `HILDA_DB_URL` (Docker Compose convention) — falls back to former, defaults to `postgresql+asyncpg://hilda@localhost:5432/hilda` if neither set. `doc_id_slug` column widened 64 → 256 chars + STR-E001 vs STR-E003 classification cascade (surfaced by qualification-workspace filename 70 chars long). Public surface should reference `get_by_customer_and_sp_id` + `list_documents_for_item_display` on next full-file curation pass. Commits: `184e82e`, `31d5e2a`, `c52777b`, `1fc4bd4`.
...
- **`TGFolderRoutingRow.item_no` referential integrity relies on `template_schema.DeliveryItemBase.item_no` immutability** (added 2026-06-12 per cross-module Invariant cascade). Per `template_schema/MODULE.md` Invariants (added 2026-06-12 SP UI engineer review): `DeliveryItemBase.item_no` is immutable per item lifetime — once assigned at template instantiation, the value never changes; SP UI reordering MUST NOT mutate it (user-visible reorder is a `sort_order` concern). This is the load-bearing assumption for `TGFolderRoutingRow.item_no` FK persistence: routing entries written at `set_folder_routing_for_tg` time refer to `item_no` by value and assume it remains addressable for the routing entry's lifetime. Without the immutability invariant, every TGFolderRoutingRow referencing a reordered `item_no` would silently break and FR-52 step 3 routing would resolve to wrong work-items. Enforcement at write: `STR-E006` (line ~402: "raises STR-E006 if any item_no is unknown"); the upstream template_schema immutability invariant prevents the runtime drift case where a previously-validated `item_no` becomes unknown post-write.
