...
- **`PauseStateLookup` Protocol-injected** — FR-31 sub-1 pause flag lives on the DeliveryItem row (authoritative home TBD per D-DRAFT-2 2026-06-12: SP column vs event-carried; `CommunicationLog` records the pause/resume audit events only). rule_engine reads via injected Protocol so unit tests can mock pause state without standing up Postgres. Same pattern as `credential_service` injection in `customer_adapter`.
