...
>
> **Rollback log:**
> - **2026-07-02 (D-141 impl surface — customer_delivery_info denormalization + ADAPTER_FACTORY convention)** — Per D-141, `customer_delivery_info` (customer-level GDrive base URL from `template.yaml` root) is denormalized per DeliveryItem at import time by `workflow_engine.tasks.sp_alert_imports._build_delivery_item` via `template_schema.template_lookup.get_customer_delivery_info(customer_id)`. Root cause of every observed CAD-E010 failure since June 24: import task never populated `customer_delivery_info` on the DeliveryItem row → `submit_to_carrier` passed empty → `GoogleDriveBaseAdapter.upload_attachment` step-0 validation raised CAD-E010 before reaching binding. Fix runs at import path (going forward) + `scripts/backfill_static_fields_from_template.py` (existing rows). **Bootstrap convention** ratified per this cascade (implementation landed in `workflow_engine.bootstrap._build_customer_adapter`): per-customer subclass at `customizations/customer_adapter/<customer_id_lower>_adapter.py` MUST expose a module-level `ADAPTER_FACTORY: Callable[..., CustomerAdapter]` returning an adapter instance when called with `audit_writer=<AuditWriter>` kwarg; discovery via `HILDA_CUSTOMER_ID` env var (default `"MMK"`). Optional module-level `bootstrap_directories(template) -> summary_dict` async method on the adapter — called at worker startup by bootstrap (default ON; opt out via `HILDA_SKIP_GDRIVE_DIRS_BOOTSTRAP=1`) to pre-create GDrive folders idempotently (create returns True on already-exists). Ph-1 credential path: **JsonFileCredentialService** at `core/src/credential_service/service.py` (plaintext-JSON `config/customer_adapter.json` — no sops for corp GDrive auth in Ph-1; Ph-2 revisit). Public surface + Sub-modules sections should reference `ADAPTER_FACTORY` convention + `bootstrap_directories` optional method + JsonFileCredentialService dependency on next full-file curation pass. Commits: `e5e955b` (JsonFileCredentialService), `9072194` (ADAPTER_FACTORY discovery), `87ee967` (bootstrap_directories default ON), `5b09af4` (customer_delivery_info denormalization at import).
