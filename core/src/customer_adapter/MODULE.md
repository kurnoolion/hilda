...
>
> **Rollback log:**
> - **2026-06-26 (TODO -- D13 cascade follow-up flagged per architect lock 2026-06-26)** — architect's live SP probe + customer_delivery_* clarification 2026-06-26 revealed that `customer_delivery_info` (e.g., "drive.google.com") + `customer_delivery_modality` (e.g., "GoogleDrive") are **PER-ROW columns on the Deliverables SP list**, NOT binding-baked customer-root config as previously assumed in D13. The `customer-baked-root` per-customer-subclass framing in D13 needs to be relaxed: instead, `customer_delivery_info` becomes a per-call arg fetched from the Deliverables row at upload time. **Follow-up cascade pending**: (a) update binding API per `[D-116]` D12 to accept `customer_delivery_info` as a 9th arg; (b) update per-customer subclass scaffolds to read `customer_delivery_info` from row context, not bake into subclass; (c) the final URL `<customer_delivery_info>/<project_model>/<milestone_name>/<target_folder>` composition happens inside the binding per (B-α) but with all 4 components flowing as args. **Also confirmed**: `customer_delivery_credential_id` REMOVED per `[D-019]` shared HILDA ops-team identity (no per-row credential needed). Scope of correction cascade: small (~50-100 lines code + MODULE.md edit + test update). Defer to next architect-review session.
>
