...
> **Status:** Initial draft 2026-06-09 (Ph-1-first scope per discipline locked 2026-06-09). Per-carrier subclass pattern + Google-Drive browser-automation baseline per `[D-054]` + `[D-054]` impl note 2026-06-05. Sections curated; code implementation begins after `/switch-phase development`.
>
> **Rollback log:**
> - **2026-06-09 (initial draft)** — final remaining Phase B Module rollback per STATUS.md In-progress 2026-06-08. Draft scoped to **Ph-1 only**: Google Drive baseline + per-carrier subclass + browser automation per `[D-054]` impl note 2026-06-05 (selenium/playwright on headless Chromium; selector versioning per-deployment; capability flags per-carrier). Ph-2 (`get_status` / `post_comment` / `fetch_feedback`) noted in `## Deferred` only. Ph-3+ (web portal flavor + JIRA-as-customer-portal flavor per `[D-054]`) noted in `## Deferred`. Anchors FR-19, FR-68 (PLM-Carrier hash sync invariant), FR-73 (carrier-package zip flow), FR-77 (target_folder outbound carrier path), FR-80 (`no_customer_upload` gate), NFR-2 (no proprietary content in compact reports). New error code prefix `CAD` per `diagnostics.PREFIX_REGISTRY`.

**Purpose**: Single Protocol-mediated surface (`CustomerAdapter`) for HILDA's outbound carrier submission — upload individual document files (per `[D-054]` — individual files only, never zips) to each carrier's submission destination + emit `CommunicationLog` entries per FR-42. **This module does NOT do FR-77 routing itself** — the caller (`workflow_engine` submission task) resolves `target_folder` via FR-77's 2-step pipeline (step 1: folder-based via `TGFolderRouting` matched on document's NSD ingress/zip-internal source folder; step 2: work-item fallback via `DeliveryItemBase.target_folder`) and passes the resolved string IN to `upload_attachment(...)`. This module just honours it. Ph-1/Ph-2 scope: **Google Drive only** per `[D-054]`. Per-carrier subclass at `customizations/customer_adapter/<carrier_slug>_adapter.py` extends the `GoogleDriveBaseAdapter` reference class in this module with carrier-specific selectors + session-login flow + capability flags (loaded from `customizations/customer_adapter/<carrier_slug>_adapter_config.yaml`). Per `[D-054]` impl note 2026-06-05 — Google Drive REST API unavailable per corp/carrier policy → **browser-automation implementation** via `selenium` (default) or `playwright` driving a headless Chromium instance authenticated as the PM via stored session cookies. Anchors `[D-007]` (on-prem; browser runs on HILDA PC inside corporate network boundary), `[D-019]` (credential_service for PM session cookies), `[D-054]` (Google-Drive-only + browser automation impl note); serves FR-19, FR-68 (PLM-Carrier hash sync), FR-73 (Ph-1 carrier-package zip two-click flow), FR-77 (target_folder outbound), FR-80 (`no_customer_upload` gate), NFR-1, NFR-2.
...
core/src/customer_adapter/
  ...
  selector_loader.py                ← loads versioned UI selectors from customizations/customer_adapter/<carrier_slug>_adapter_config.yaml
...
customizations/customer_adapter/
  <carrier_slug>_adapter.py         ← per-carrier subclass: overrides login flow + selector pack + capability flags
  <carrier_slug>_adapter_config.yaml ← per-carrier configuration: Drive folder root path, selector versions, capability flags, session-cookie storage key
...
class CarrierCapabilityFlags:
    ...
    customizations/customer_adapter/<carrier_slug>_adapter_config.yaml. Some Google Drive
    ...
    supports_hash_verification: bool = False # FR-68 — Ph-1 Google Drive does not expose file hash via UI; Ph-2 may add via API discovery
...
class CustomerAdapter(Protocol):
    ...
    async def upload_attachment(
        ...
        target_folder:       str,              # FR-77 outbound carrier path (per-item DeliveryItemBase.target_folder)
        carrier_credential_id: str,            # opaque reference to credential_service-stored session cookie blob
...
class GoogleDriveBaseAdapter(CustomerAdapter):
    ...
    Per-carrier subclasses (e.g., `customizations/customer_adapter/<carrier_slug>_adapter.py`)
    extend by:
    ...
    Browser automation: selenium (default) or playwright per `Fr52Config.browser_engine`;
    ...
    async def upload_attachment(
        self, file_path: Path, target_folder: str, carrier_credential_id: str,
        ...
        5. Wait for upload-progress completion per selector_pack.upload_done_indicator (timeout per Fr52Config).
...
class SessionManager:
    ...
    Sessions expire per `Fr52Config.session_max_age_s` (default 3600s); expired sessions
    are torn down + re-established on next use."""

    def __init__(self, max_sessions_per_carrier: int = 1, max_age_s: int = 3600) -> None: ...
    async def acquire(self, carrier_slug: str) -> "BrowserSession": ...
...
class SelectorPack:
    ...
    customizations/customer_adapter/<carrier_slug>_adapter_config.yaml. Selector-version bumps
...
def load_selector_pack(carrier_slug: str, config_dir: Path) -> SelectorPack:
    """Loads from customizations/customer_adapter/<carrier_slug>_adapter_config.yaml.
...
class MockCustomerAdapter:
    ...
    CarrierUploadResult per registered (carrier_slug, target_folder) pair."""

    source_system: str = "mock_customer"

    def register_upload_result(
        self, carrier_slug: str, target_folder: str, result: CarrierUploadResult
    ) -> None: ...
...
Credentials: `credential_service.get_credential(pm_id, SystemType.CUSTOMER)` returns the PM's per-carrier session-cookie blob; per `[D-019]` Ph-1/Ph-2 model, this is the ops-team-provisioned shared session.
...
- **PLM-Carrier hash sync per FR-68** — every file uploaded to carrier must hash-match a file present on PLM (top-level OR member of a PLM-resident zip). Non-match emits `ITR-W003` from `issue_tracker` post-dispatch verification. Ph-1: hash-from-PLM side only (`file_hash_carrier` may be None on Ph-1); Ph-2 may add carrier-side hash extraction per `CarrierCapabilityFlags.supports_hash_verification`.
...
- **No credential material in logs / reports** per NFR-2 / `[D-002]`. Session cookies retrieved per-session via credential_service; never logged, written to disk, or echoed to compact reports. `carrier_credential_id` is an opaque reference only.
...
```
CAD-E001  Selector config file '{path}' not found for carrier '{carrier_slug}' (selector_loader)
...
CAD-E004  Browser session establishment failed for carrier '{carrier_slug}': {reason} (cold-start timeout, login failure, etc.)
CAD-E005  Upload timed out after {timeout_s}s on file_hash='{file_hash}' carrier='{carrier_slug}' target_folder='{folder}' selectors_version='{version}'
...
CAD-E007  Carrier file_id extraction failed post-upload — uploaded file may exist on carrier side; manual verification needed (file_hash='{file_hash}' carrier='{carrier_slug}')
CAD-E008  Carrier credential '{credential_id}' not retrievable from credential_service
CAD-W001  Capability flag '{flag}' is False for carrier '{carrier_slug}' — operation '{op}' not supported; caller fell back gracefully
...
CAD-W004  Selector-pack version mismatch: subclass expects '{subclass_version}', YAML provides '{yaml_version}' (carrier '{carrier_slug}') — proceeding per caller policy
```
...
- **Session pool per carrier** — Chromium cold-start dominates per-upload latency. One reused session per carrier (default; configurable) amortizes the cost. Sessions auto-expire per `Fr52Config.session_max_age_s`; re-established on next use.
...
- **`credential_service.SystemType.CUSTOMER`** — single SystemType value for all carriers (vs per-carrier SystemTypes). Per-carrier credential differentiation handled via `carrier_credential_id` lookup key within the CUSTOMER credential blob. Matches credential_service pattern for "one SystemType per system kind, not per system instance".
...
- `workflow_engine` — fires `QueueSubmission` action which invokes `upload_attachment` per item via Celery task; gates per `DeliveryItemBase.no_customer_upload`; advances `delivery_state` to `SubmittedToCustomer` on success per FR-7.
- `dashboard` — surfaces upload status from `CommunicationLog`; renders carrier file URL link (if returned in `CarrierUploadResult`) for TPM verification.
- `issue_tracker` — FR-68 PLM-Carrier hash sync verifier consumes `CarrierUploadResult.file_hash_local` post-dispatch to verify against PLM-resident attachments.
...
```
Fields: carrier_slug (enum: registered carrier slugs), selectors_version (str — bounded
