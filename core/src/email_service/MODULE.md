...
2. **Short-interval polling fallback** — 5–10 s when IDLE unavailable.
...
**Source-of-truth for email content**: Inbound email body + attachments + headers are stored once at receipt time in NSD `<tg>/<item>/email/<message_id>/` (audit) and indexed in `CommunicationLog` (Postgres). Mailbox is treated as a transient queue — once processed and acknowledged in `CommunicationLog`, the email is marked-read in IMAP. Re-fetching is by `CommunicationLog.message_id` lookup, not by mailbox re-poll.
...
**Test-scenario minimum subset** (per user 2026-05-28 use case — incoming email parsing for 80+ test reports with structured response, no LLM review, no revisions): `inbound/receiver.py` (one-shot poll mode), `inbound/classifier.py`, `inbound/subject_parser.py`, `inbound/body_parser_structured.py`, `inbound/attachment_router.py`. **Skipped for test scenario**: `body_parser_freetext.py` (FR-12 path c — fused LLM call), entire `sp_alert_parser/`, entire `outbound/`.
...
class RoutedAttachment:
    ...
    classification_source:   Literal["filename_rule", "tier1_fuzzy", "tier2_llm", "staged_pm"]
