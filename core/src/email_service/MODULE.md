      ...
      **Parallel branches** (independent per FR-85; ordering and parallelization are
      ...
      Branch A — **FR-85 doc_type classification (2-step ladder)**:
        Step A1: Filename regex (FR-85 Step 1)
          - Load per-customer rules from `customizations/<customer_id>/doc_type_filename_rules.yaml` (per `[D-091]`)
...
**FR-87 strict A→B→C TPM resolution action handlers (added 2026-06-09 per `[D-053]` impl note 2026-06-08; updated 2026-06-25 for `[D-117]` + `[D-118]` + `[D-119]`)**: When TPM saves a resolution via the SP UI, the SP-alert email carries an action verb in the sub-header (per FR-84). The parser routes to one of these handlers based on action_type recognized in the body key:value. Handler SP writes (where applicable) flow through `sharepoint_integration` per `[D-117]` SP NTLM digest-dance pattern: requests-ntlm + lazy digest + MERGE pseudo-verb + `__metadata` wrapper + `_x005f_`/`_x0020_` encoding. `email_service` does NOT call `SpClient` directly — it triggers `tracker` / `workflow_engine`, which call `sharepoint_integration`:
- **`tpm_reassign_to_workitem`** (FR-87 step A; also fires for FR-83 default→real reassignment) — handler: storage.reassign_document_to_workitem(file_hash, source_delivery_item_id, target_delivery_item_id, pm_id); HILDA re-runs FR-86 storage matrix → moves file between paths; runs `[D-039]` revision determination if gate now permits. Resulting SP writes flow through `[D-117]` digest-dance.
- **`tpm_resolve_doc_type`** (FR-87 step B — NEW 2026-06-09) — handler: storage.update_doc_type(file_hash, target_doc_type, pm_id); HILDA validates FR-86 alignment invariant against current item_type; re-runs FR-86 storage matrix; runs `[D-039]` if doc_type just became non-UNRESOLVED and item is non-Default; emits FR-77 carrier-upload trigger if file reaches classified path. **`tpm_resolved_doc_type` SP field per `[D-119]` accepts only 4 values — `DocType` MINUS `unresolved` (`{test_report, tech_report, waiver, compliance_certification_release_notes}`); TPM cannot select `unresolved` (HILDA's classifier-failure sentinel — not TPM-meaningful)**. HILDA owns the FULL lifecycle (write + read) of `tpm_resolved_doc_type` via FR-87 step (B) HILDA-rendered web page per `[D-074]`. SP write goes through `[D-117]` digest-dance.
- **`tpm_resolve_revision`** (FR-87 step C — NEW 2026-06-09) — handler: storage.set_revision_resolution(file_hash, verdict ∈ {NEW_DOCUMENT, REVISION_OF}, target_doc_id_slug, pm_id); HILDA assigns doc_id_slug + rev_number; moves file from `_staged_revision/` to canonical classified path; emits FR-77 carrier-upload trigger. SP write goes through `[D-117]` digest-dance.
...
class Fr52Config(BaseModel):
    ...
    doc_type_filename_rules_path:            Path = Path("customizations/<customer_id>/doc_type_filename_rules.yaml")   # FR-85 Step 1 — per-customer YAML per `[D-091]`; universal fallback at `core/src/email_service/default_doc_type_rules.yaml`
