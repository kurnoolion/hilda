...
class Fr52AttachmentRouter(AttachmentRouter):
    ...
    **Per-attachment pipeline (revised 2026-06-09)**:
    ...
    def __init__(
        ...
        route_attachment_max_matches_threshold:  int = 3,                      # FR-79 — count-based threshold for EML-W007 / LLG-W008 (changed 2026-06-09 from float summed-confidence per user review — count is more interpretable)
...
class Fr52Config(BaseModel):
    ...
    route_attachment_max_matches_threshold:  int   = 3      # FR-79 — max number of matches per inbound document considered "normal"; above (e.g., N > 3) → emit LLG-W008 / EML-W007 for ops visibility (all matches still committed per FR-79 contract). Count-based (changed 2026-06-09 from summed-confidence float threshold per user review: count is more interpretable and discriminative than summed confidence — typical multi-item docs cover 2-3 items max; 5+ matches is a likely over-routing signal regardless of per-match confidence).
...
EML-W007  Attachment over-routed per FR-79 — N={n} matches on file_hash='{file_hash}' exceeds `Fr52Config.route_attachment_max_matches_threshold` (default 3); all matches committed per `[D-055]` symmetric M:M contract but flagged for ops review (recoverable; added 2026-06-09; count-based threshold per user review)
```
