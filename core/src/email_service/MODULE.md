...
> **Status:** Draft + 2026-06-09 cascade Group 4 of 4 + **2026-06-25 Module #12 arch revisit (D1-D13 against locks since 2026-06-09)** + **2026-06-25 EVENING [D-132] EWS adapter (corp Exchange path; mode discriminator)** applied (`[D-053]` impl note 2026-06-08 corrected model: 5-value DocType + alignment invariant + FR-85 2-step classification ladder + FR-86 4-path storage matrix + FR-87 SP UI A→B→C resolution; layered alignment to `[D-091]` customer_id, `[D-094]` SUPERSEDED item_type mixed-case, `[D-104]` Projects per-customer, `[D-105]` 4-field owner identity, `[D-106]` TGGroupBase dropped, `[D-107]` credential scope, `[D-108]` rules_paused, `[D-113]` TriggerDispatcher, `[D-115]` dashboard-local auth, `[D-116]` customer_adapter thin-wrapper, `[D-117]` SP NTLM digest-dance, `[D-118]` SP UI engineer provisioning, `[D-119]` tpm_resolved_doc_type 4-value). Initial draft 2026-05-28. Sections curated; pending section-by-section user review before contract is finalized. Code implementation begins after `/switch-phase development`.
>
> **Rollback log:**
> - **2026-07-02 (parser Milestones-trigger promotion + D-143 SP-alerts-lossy stance)** — `sp_alert_parser` gains `_MILESTONE_PROMOTE_FIELDS` tuple in `_extract_field_deltas` handler: when list_name is Milestones AND CHANGED alert body has non-empty value for any of `milestone_collection_started_at` / `milestone_submission_triggered_at` / `closed_all_items_triggered_at` but the value did NOT carry an "Edited" marker (SP renders no-marker in some CHANGED-alert forms per architect Q3 lock 2026-06-30), promote the field into `field_deltas` so downstream rule engine can match on `neq null` conditions. Extension of prior 2026-06-30 "kickoff milestone_collection_started_at promotion" fix — this cascade generalizes to a tuple + adds the third trigger field for close-all-items per D-141/D-142. Parser also handles dash-less Edited-value form (`key: <old_value> <new_value> Edited` in addition to `key: - <new_value> Edited`) surfaced by architect live test 2026-06-29. Layer-2 content-hash dedup Layer already lands per prior D-133. `_extract_body_html` widens body_html acceptance (isinstance HTMLBody OR HTML marker check — Outlook reply downgrades BodyType=Text while preserving HTML markup). Owner-reply parser drops literal `HILDA-BATCH-ID:` regex + uses substring check per D-136 (Outlook quoted-history content defeats regex; substring is Outlook-rendering-agnostic). **D-143 architectural stance** ratified this session: HILDA officially treats SP alerts as best-effort notification signal, not authoritative delivery contract. Email is fast-path (sub-minute latency when it works, covers 85-97% of alerts); reconciliation cascade per D-142 is authoritative backstop (5-min drift window at worst). Every SP-driven state transition needs BOTH an email-path handler AND a reconciliation sweep. Invariants section should reference `_MILESTONE_PROMOTE_FIELDS` tuple + D-143 stance on next full-file curation pass. Commits: `62a3222`, `a4e375f`, `24eae60`, `2457af4`, `df0f7b3`, `8a60abb`.
...
core/src/email_service/
  ...
  inbound/
    ...
    ews_receiver.py                 ← `[D-132]` 2026-06-25 — Exchange Web Services receiver via exchangelib (corp Exchange deployments; basic auth + service account; polling Ph-1; streaming notifications Ph-1 next pass)
  ...
  outbound/
    ...
    ews_sender.py                   ← `[D-132]` 2026-06-25 — Exchange Web Services sender via exchangelib Message.send_and_save (corp Exchange deployments; sync wrapped via asyncio.to_thread; per-call credential resolution)
...
class PerItemReplyUpdate:
    item_no:           int
    delivery_state:    str | None        # "Open" / "OwnerClosed" / "Delayed" / "Blocked" / etc. (D-124 α: PascalCase + space)
...
EML-E008  EWS authentication rejected (credential_service may need refresh) — added 2026-06-25 per `[D-132]`
EML-E009  EWS transport failure: {reason} — added 2026-06-25 per `[D-132]`
...
- **`[D-016]`** — IMAP/SMTP for the mailbox channel; rejected EWS / Graph API as they fail NFR-1 (no SaaS LLM is unrelated, but the Graph endpoint adds external surface area). IMAP IDLE remains pending Exchange-admin confirmation per STATUS.md Flag. **PARTIALLY SUPERSEDED 2026-06-25 by `[D-132]` for corp Exchange deployments**: corp Exchange has IMAP/SMTP DISABLED at the server; the only mail wire path is on-prem EWS (no SaaS Graph; NFR-1 still honored). IMAP/SMTP path retained for non-corp + mock + dev deployments; `EmailServiceConfig.mode` picks at runtime.
- **`[D-132]`** — EWS adapter for corp Exchange (added 2026-06-25). Corp Exchange (Build 15.2 per Chaitanya Kamsu's validated `ExchangeMailService` sample) does not expose IMAP/SMTP; the only mail wire path on-prem is Exchange Web Services. Adapter pair: `inbound/ews_receiver.py` + `outbound/ews_sender.py` implementing the existing `EmailReceiver` + `EmailSender` Protocol surfaces (no Protocol changes — adapters are wire-format-substitutable). Mode selection via `EmailServiceConfig.mode: Literal["imap_smtp", "ews", "mock"]` with factory functions `build_receiver(cfg, cred)` + `build_sender(cfg, cred)` in `email_service/__init__.py`. Library: `exchangelib>=5.2,<6` (lazy-imported so non-EWS deployments don't pay the cost). Authentication: basic-over-TLS with a service account; `autodiscover=False`; explicit `service_endpoint` URL. Inbound mechanism: **polling Ph-1** (cadence = `EwsConfig.poll_interval_s`, default 60s); EWS streaming notifications (`exchangelib.SyncFolderItems`) deferred Ph-1 next pass — adds long-lived TCP + reconnect retry complexity not justified by latency requirements (FR-23 deadline-tiered polling already covers Ph-1). Reference pattern: Chaitanya's `Configuration(credentials, service_endpoint, auth_type='basic', version=Version(Build(15,2)))` + `Account(primary_smtp_address, autodiscover=False, access_type=DELEGATE)` + `account.inbox.filter(Q(datetime_received__gt=...))`. Attachment write-to-disk does NOT happen at the receiver (attachments stay in-memory as bytes on `InboundAttachment.content` — FR-86 storage matrix is the only legitimate landing site). Errors: `EML-E008` (EWS auth rejected), `EML-E009` (EWS transport failure). Retries: no `@retry` decorator on `_send_sync` (workflow_engine is source of truth for retries per `[D-022]`; double-layered retry is anti-pattern).
