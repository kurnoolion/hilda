...
class DeliveryItemBase(_Base):
    ...
    item_type:                       str
    # 4-field owner identity per FR-88 + [D-080] + [D-086] (free-form, no AD
    # validation). OWNER-7 (2026-08-16, [D-172] B-final-B): these unsuffixed
    # names now carry LIST values instead of the original str | None. Ingest
    # populates via _split_owner_list on the semicolon-separated SP text
    # field ("alice@corp; bob@corp;"). All 4 lists are index-aligned per
    # person -- empty string in a slot for unknown attributes. Empty list
    # == no owner (was NULL pre-OWNER-1).
    owner_corp_usa_email:            list[str] = Field(default_factory=list)
    owner_corp_email:                list[str] = Field(default_factory=list)
    owner_corp_id:                   list[str] = Field(default_factory=list)  # PLM grouping key per FR-5 + [D-035]; assignee = [0]
