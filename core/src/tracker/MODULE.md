...
>
> **Rollback log:**
> - **2026-07-02 (D-137/D-138/D-139/D-140 cascade — SP writeback + Guard 4 trigger_source trust)** — Landed pre-window: `tracker.transitions.update_delivery_state` two-step natural-key → `_sp_id` lookup before `sp_writer.update_item` per [D-137] (SP REST `/items({Id})` expects integer auto-counter Id, not HILDA's composite `<customer>-<device>-<milestone>-<item_no>` slug); state enum value strings verbatim to SP UI Choice column per [D-138] (OutreachSent / DocumentReceived / OwnerClosed / UnderPMReview / ReadyForSubmission / SubmittedToCustomer — was space-bearing). In-window: **Guard 4 trust-list per D-140** — `_carrier_upload_complete` guard now trusts `trigger_source='submit_to_carrier_task'` (and by extension `'sync_backfill_submit_to_carrier'` per D-142 reconciler) as authoritative evidence of upload completion. The task IS the authority — per-item transitions happen only after `all_ok and files_ok > 0` at the task-body level. Rejected adding a real `carrier_upload_complete` column: (i) derived data (function of upload attempts + all-files-success), storing adds sync burden; (ii) reconciler-driven syncs would still need trigger_source discrimination anyway; (iii) test surface simpler when "authority" is a code path, not a data column. Establishes pattern for HILDA-authoritative transitions (companion to D-139 SP-authoritative Pattern A). Companion PM approval Pattern A per D-139: HILDA reads SP CHANGED alert's 3-field atomic write (`delivery_state=ReadyForSubmission` + `pm_approval_at` + `pm_approval_pm_id`) and mirrors to Postgres via `workflow_engine.tasks.pm_approval.apply_pm_approval_task` WITHOUT invoking state-machine guards (SP is authority). Owner-intent race resolution wired: `owner_intent_closed_at` field cleared on OwnerClosed entry; new rule `reconcile_owner_intent_on_doc_count_reached` handles the race between doc_count reaching threshold + owner intent close. Key choices + Invariants sections should reference Guard 4 trust-list + Pattern A boundary on next full-file curation pass. Commits: `f9542fa` (D-137), `3201761` (D-138 cascade), `2ca7a2a` (Pattern A initial), `5cbc383` (Guard 4 trust).
...
- **`[D-118]` strict-boundary cascade 2026-06-26 — HILDA no longer writes Default WI row to SP.** Rejected prior choice: `instantiate_default_workitem` was the one row-creation case where HILDA wrote to SP (per `[D-064]`); other SP rows were created by SP UI engineer's setup_milestone flow. After architect direction 2026-06-26 PM ("SP UI engineer is the single owner of SP rows"), the cascade removes the `sp_writer.create_item` call from `default_workitem.py`; SP UI engineer creates the Default WI row in SP via the "Setup Deliverables" button alongside the 10 template-driven Deliverable rows; HILDA imports it via `workflow_engine.tasks.sp_alert_imports.import_deliverable_tracker_task` when the SP ADDED alert arrives. `instantiate_default_workitem` is retained for the direct-invocation path (tests + dev-mode setup); it writes only to local storage with a HILDA-synthesized composite-key id (`{customer_id}-{device_id}-{milestone_id}-default`) and never calls `sp_writer.create_item`. Boundary preserved: tracker still never writes to SP directly; the change removed the one prior exception. Sole owner of SP row creation is now SP UI engineer per `[D-118]`. Cascade ratified as `[D-135]` 2026-06-28 after all 5 chunks landed + live-validation on real corp Exchange+SP.
