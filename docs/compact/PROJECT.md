...
- **Three-tier LLM access model** — three distinct LLMs with non-overlapping access:
  ...
  2. **On-prem code-generation LLM (Gemma3:12b / Qwen / similar — open source, configurable)** — used by two Ingestors that both run on company premises and emit into `customizations/`: (i) **API Spec Ingestor** `[D-003]` — proprietary issue-tracker / messenger API specs → adapter code; (ii) **Template Schema Ingestor** `[D-010]` — proprietary customer-template Excel schemas → Pydantic validators, Excel parsers, SharePoint-List column mappings, customer-specific AutomationRules. Each Ingestor has its own diagnostic CLI emitting compact reports the dev can paste into Claude-chat to debug ingestion / generation without exposing the proprietary content.
...
- The dev LLM **never** reads proprietary API specs (`[D-003]`) or proprietary customer-template schemas (`[D-010]`). Phase-prompt enforced.
...
| PMs (primary end users) — names TBD | Bug reports, UX feedback, eval data (correct AI classifications, edit AI drafts) | DeliverableHub SharePoint UI + issue tracker | Telemetry + ticket triage |
