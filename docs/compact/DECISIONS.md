<!-- GENERATED by scripts/sanitize_docs_for_public.py -- do not hand-edit.
     Corp specifics are replaced with stable placeholders: device models map
     1:1 to SM-DEVICE<X>, so cross-references stay coherent. Edit the source
     under docs/compact/ and re-run. -->
...
| `pm_email` | `Work email` | `<owner-email>` | FR-25 (b) CustomerJIRA self-outreach recipient; outreach signature reference (HILDA From stays team mailbox per FR-23) |
...
- (b) `User name` as `assigned_pm_id` preferred over email-local-part: PMs may be partner/contractor identities (e.g., `@<corp-host>`) whose email-local-part can be ambiguous; `User name` is the SP-supplied corp directory identifier and is the stable key. Email-local-part fallback retained for robustness when `User name` is null (rare).
...
Concrete example: `drive.google.com/OEM_Folder1/OEM_Folder2/SM-DEVICEC/P1/Compliance/Sustainability`.
...
- **Q2**: `exchangelib` over raw SOAP / `suds-jurko` (matches <colleague> proven sample)
...
- (e) **exchangelib over raw SOAP** -- <colleague> corp-validated pattern uses exchangelib; mirroring it avoids re-deriving 5+ years of EWS-quirk handling. Library has explicit Build/Version helpers + correct DELEGATE semantics + native attachment + filter Q-object support. Raw SOAP would be 3x the line count + harder to maintain.
...
**Anchors**: `[D-016]` (IMAP/SMTP partially superseded), `[D-022]` (workflow_engine retry source-of-truth), `[D-025]` (3-tier config + hot-reload), `[D-038]` (sops-encrypted credentials), `[D-107]` (credential_service scope-aware; EMAIL stays SHARED), <colleague> `ExchangeMailService` reference (validated against current corp Exchange Server), NFR-1 (no SaaS LLM / external surface; on-prem EWS honors), NFR-2 (no credential material in logs; per-call credential resolution), FR-9 / FR-10 / FR-12 / FR-23 / FR-24 (mailbox channel callers; Protocol-substitutable), `core/src/email_service/MODULE.md` Key choices, commits from 2026-06-25 evening Phase D2 + EWS landing session.
                          timeout_s: float | None = None) -> FileRef: ...
    async def react(self, ref: MessageRef, emoji: str,
                    timeout_s: float | None = None) -> None: ...
    async def register_webhook(self, callback_url: str, secret: str,
...
- (f) HILDA_DASHBOARD_REVERSE_PROXY_ORIGIN load-bearing for WOPI URL construction; must be set at deploy (http://<hilda-host>:8443).
...
**Context**: Multi-session confusion resurfaced during the SM-DEVICEB corp-box test cycle: is `no_customer_upload` a HILDA-outreach gate, a submit-to-carrier gate, or both? What are the correct per-item-type + per-milestone values? Ambiguity was masking a template.yaml data bug (D-141 tension flagged separately) and also masking a design question: does outreach send to items with `no_customer_upload=True` or `False`? Architect clarified the intent verbatim during 2026-08-03 debugging session.
...
Per SM-DEVICEB corp-box test cycle investigation 2026-08-03: `_tmpl_bool(tmpl, key, body_kvs, default)` precedence (template value wins if key present, even False; body_kvs is fallback; model default is last resort) protects against SP null-glitches at setup time BUT also silently overrides correct SP values with wrong template values. Concrete evidence: MMK/SM-DEVICEB/DRR imported with `force_tracking_enabled=false` + `no_customer_upload=false` on 84/86 items because template.yaml had those flags as False; SP's Yes/False values lost. Blocked kickoff outreach (only item#86 got the email); root-caused via RTRC-1 route-trace + template_lookup direct query.
...
## D-180: Revision-family identity via conservative version-token stripping; Step C ungated without a new flag

**Status**: Active · **Date**: 2026-08-31

**Context**: An owner resending a document produced a broken index state. The router's Step C multi-revision lookup sat behind `ph1_first_pass_substring_only=True`, so `rev_number` was ALWAYS 1 and `doc_id_slug` was the raw filename stem. Two failure shapes: a **same-filename** resend produced a second `document_index` row colliding with the `uq_doc_slug_rev` unique index `(milestone_id, doc_id_slug, rev_number)`, AND `internal_classified()` re-derived the SAME NSD path so revision 2's bytes overwrote revision 1 on disk; a **different-filename** resend (`report_v2.xlsx`) slugged differently and forked into an unrelated family, so nothing downstream could tell the two documents apart. User's stated requirement 2026-08-30: "when version 2 of a doc is sent, irrespective of same file name or different file name, the doc_id_slug should be the same… if owner sends a completely different file name it will be treated as a different document. The concept of doc_id_slug was introduced to address the same document sent by owner with slightly different suffix like v3 or v2, but largely the file name remains the same."

Options for enabling revision families:
- **Option A (flip `ph1_first_pass_substring_only` to False)**: the flag already gates the Step C lookup. But the flag is overloaded — it ALSO gates Steps B2/B3/B4 (fuzzy match, folder routing, LLM routing) and the `TG_SINGLE_ITEM` Stage 0 shortcut. Flipping it would re-enable the 2026-07-25 `TG_SINGLE_ITEM` mis-routing regression, whose STATUS Flag records that restoring it safely requires owner-scoped candidate filtering that is not implemented.
- **Option B (add a second, narrower flag)**: e.g. `multi_revision_enabled`, gating only Step C. Isolated blast radius but adds a flag.
- **Option C (ungate Step C only, no new flag)**: delete the `if not self._ph1_first_pass_substring_only:` guard around the Step C lookup, leaving the flag and its other three gate sites untouched.

Options for matching filename variants to a family:
- **Option D (fuzzy similarity, e.g. rapidfuzz)**: already a project dependency. Tolerant of arbitrary variation.
- **Option E (strip a closed set of trailing version tokens, exact-match the stem)**: deterministic, explainable, no new dependency.

**Decision**: Option C + Option E, per user direction 2026-08-31 ("suggest no need of new flags as we wont go back… make code change without bloat of new flags") and 2026-08-30 ("keep it simple without rapidfuzz dependency").

1. `_slug_from_filename` strips trailing version tokens before slugging: a trailing `(N)` counter removed from the raw base name before normalization, then a trailing `_v<N>` or `_rev<N>` stripped repeatedly from the normalized slug (so `report_v2_rev3` collapses). Token set locked by the user 2026-08-30 as `(_v2, v3, _rev2, (1))`. Result: `report.xlsx`, `report_v2.xlsx`, `report v3.xls`, `report_rev2.docx`, `report (1).xlsx` all slug to `report`.
2. A bare trailing `_<digits>` is deliberately NOT stripped.
3. Step C ungated; `ph1_first_pass_substring_only` stays `True` and continues to gate the other three sites.
4. Family matching normalizes BOTH sides before comparison but keeps the STORED slug spelling as the family key.
5. New storage read path `get_max_rev_for_slug(milestone_id, doc_id_slug) -> int` (0 when the family has no resolved rows; staged rows with NULL `rev_number` excluded, matching `list_revisions`' contract). Family hit → `rev_number = max + 1`; miss → new family at rev 1. Either way `slug_determined=True`, so Step D dispatches CLASSIFIED rather than bouncing the file to `_staged_revision`.

**Why**:
- **Ungating only Step C avoids re-opening a known live regression.** The flag's four gate sites are unrelated concerns that drifted into one boolean. Option A would have turned on fuzzy matching, LLM routing and the promiscuous TG=1 shortcut as a side effect of wanting revision families — the `TG_SINGLE_ITEM` shortcut had already caused a live corp mis-route onto item_no=84.
- **No new flag, because the old behaviour is not coming back.** User's reasoning verbatim: "we wont go back". A second flag would need a default, a deploy story, and eventual removal, for a behaviour nobody intends to revert.
- **Exact-match-on-stripped-stem is auditable; fuzzy matching is not.** A TPM can look at two filenames and predict whether HILDA will treat them as one document. A similarity threshold cannot be reasoned about without running it, and a wrong merge silently destroys a revision by overwriting a family winner.
- **The `_<digits>` exclusion is the load-bearing conservatism.** Real filenames in this deployment carry date-ish tails — `PA3_Software-Hardware Release Notes_SWMK_V4_1104.docx` appears in STATUS Flags. Stripping `_1104` would merge `..._V4_1104` with `..._V5_1105`, two genuinely different documents. The asymmetry is deliberate: **a missed merge is visible to the TPM (two rows where they expected one) while a wrong merge is invisible (one row silently replacing another).** Consequence accepted: an owner using an unlisted suffix (`_final`, `_updated`, `-B`) opens a separate family and both files upload.
- **Preserving the stored slug spelling avoids orphaning legacy rows.** Rows written before this change carry un-stripped slugs (`report_v2`). Rewriting the key would break their `uq_doc_slug_rev` index entries and the `(milestone_id, doc_id_slug)` family lookups that reference them.
- **Both storage lookups degrade to NEW_DOCUMENT on error rather than staging the file.** Forking a family is recoverable by a TPM; stranding a document in `_staged_revision` is not.

**Consequences**:
- **`document_index` rows for the same document now legitimately differ in `rev_number`.** The `uq_doc_slug_rev` collision and the on-disk overwrite both disappear because `internal_classified()` derives a distinct `rev<N>/` path per revision.
- **Cross-channel families still do not unify.** NSD ingest passes the share-relative path as the filename so its slug includes the folder, while email passes a bare basename — the same document arriving on both channels forms two families. Tracked as `SLUG-CHANNEL-1` in STATUS Flags.
- **NSD family membership requires same-folder.** Because the folder is in the slug, `2. DMDform (Done)/report.xlsx` and `3. Other/report_rev2.xlsx` are different families.
- **Mid-name version tokens are preserved as identity.** `DMD_M3_IMEI_1_V2_1107_HW update.xlsx` keeps its `V2`; only a trailing token strips.
- **One extra DB query per classified attachment** (`find_doc_id_slugs_for_item`, previously skipped under the flag) plus one `get_max_rev_for_slug` on a family hit. Accepted for the ingest path.
- **Downstream consumers must now select a family winner.** Iterating associations no longer means iterating documents — directly motivated D-181.
- 27 tests (`test_revision_families.py`), including the negative cases that pin the `_<digits>` exclusion.

**Anchors**: FR-17 / `[D-039]` (multi-revision handling, previously Ph-2); FR-57 (`doc_id_slug` stable across revisions, `rev_number` distinguishes); FR-86 (storage matrix — `slug_determined` drives CLASSIFIED vs staged dispatch); `[D-041]` (NSD `revN/` path convention); `[D-151]` + STATUS Flag 2026-07-25 (`TG_SINGLE_ITEM` regression the flag protects); D-181 (upload-side winner selection this enables); commit `e9f6436`.


## D-181: Carrier upload resolves two axes — one file per revision family, at its current view-tree version

**Status**: Active · **Date**: 2026-08-31

**Context**: `submit_to_carrier` iterated `list_classified_associations_for_item` and uploaded each row's `local_nsd_path` (the internal-tree file). Two defects fell out, both surfaced while scoping the DRR→P1 work:
- **Every revision uploaded.** An owner resend creates a new `document_index` row and a new `document_item_association`, so a resent document was pushed to the carrier twice under the same filename.
- **The as-received bytes uploaded.** A TPM browser edit (WOPI save) writes a new `document_version` row in the view tree and deliberately creates NO new association (MTR-1's stated design intent: "v2+ don't create new assocs — assoc is keyed on v1 hash"). So the association still pointed at the ingest-time file and **the TPM's edit never reached the carrier.**

The user's mental model was that uploads already resolved through `document_version` with `is_current` selecting the version — the code did not do this, which is doc/model drift in the opposite of the usual direction. Options presented and weighed with the user:
- **Option A (build DRR→P1 on the existing internal-path basis)**: smaller, isolated change; inherits both defects for migrated and native documents alike.
- **Option B (fix the upload source first, then build DRR→P1 on top)**: changes what physically reaches the carrier for every submission; needs its own testing and ADR.

**Decision**: Option B, chosen explicitly by the user 2026-08-30 ("(B) — to fix this first before moving on with DRR to P1 migration task"). New `storage.list_upload_files_for_item(delivery_item_id) -> list[ItemUploadFile]`:
1. **Revision axis** (`document_index`) — group the item's classified associations by `doc_id_slug`; keep one winner per family: `is_final` when a TPM has pinned one per FR-66, otherwise the highest `rev_number`.
2. **Version axis** (`document_version`) — resolve the winner's `file_hash` to its view-tree path; the file at that path IS the current version, so a TPM edit ships with no special-casing.
3. The winner is matched against **all** of a path's version `sha256` values, not just the current one, because a TPM edit has a fresh hash and no index row. Same join `refresh_manual_triage_after_view_save` uses.
4. Filtered inside the resolver rather than at the call site: `doc_type == waiver` (never uploaded, any milestone) and archive containers (both the `ARCHIVE_CONTAINER` sentinel and an archive basename).
5. Two fallbacks: no view-tree row for a winner → upload the association's internal path (`is_view=False`); resolver absent or raising → the task adapts raw associations to the same surface and proceeds with the old every-revision behaviour, logged.

**Why**:
- **The two axes are independent and both are needed.** Family selection answers "which document"; `is_current` answers "which version of that document's file". Conflating them is what produced both defects. Neither axis alone is sufficient: revision-only ships stale bytes after a TPM edit, version-only ships every revision.
- **Fixing upload before building on it avoids writing the DRR→P1 resolution twice.** The migration path needs exactly this resolution; building it on the association walk would have meant reimplementing the dedup once the base was fixed.
- **Waiver exclusion belongs in the resolver, not the call site.** User lock 2026-08-31: "waivers are never uploaded irrespective of DRR or P1 milestone." Enforcing it at one chokepoint means the rule cannot be missed by a future caller — and it is what makes the DRR→P1 migration inherit the rule for free.
- **Fallbacks keep the blast radius bounded.** Items with no `tg_name`, the Default WI, and pre-D-150 documents have no view-tree presence; without the internal-path fallback they would have silently stopped uploading. The resolver-absent fallback is why all 11 pre-existing `TestSubmitToCarrier` tests passed unmodified.
- **Verified on live corp data** before relying on it: item 108 held three classified associations across two families and resolved to two uploads, with the family-1 winner resolving to the path whose current version was a TPM edit.

**Consequences**:
- **What physically reaches the carrier changed for every submission**, not just migrated documents. A resent document now uploads once; a TPM-edited document now uploads the edit.
- **`ItemUploadFile` becomes the upload-side contract**, replacing raw `DocumentItemAssociation` rows in `submit_to_carrier`. Later grew `from_zip` (D-183) and `migrated_from_*` (D-185).
- **The carrier sub-folder derivation moved to the view path**, which immediately caused a regression — NSD folder names leaked to Drive. Fixed by D-183 the following day.
- **`is_final` (FR-66) is now load-bearing but unexercised.** No evidence in STATUS or the code that TPMs set it; the branch is inert until they do. Flagged rather than assumed.
- **The two-revisions-one-path case is handled.** A same-filename resend puts both revisions on one view path; family selection collapses them to one upload.
- **Verified only against SQLite** in tests; the resolver leans on `IN` clauses and a three-table join.
- 25 tests (`test_upload_view_resolution.py` + 3 wiring tests).

**Anchors**: D-180 (revision families this consumes); FR-66 (`is_final` version selection); D-150 (view tree + `document_version`); MTR-1 / MERGE-1 (the sha256-membership join reused here); `[D-116]` / `[D-054]` (individual-file carrier upload, never zips); D-183 (sub-folder provenance fix); D-185 (cross-milestone reuse); commit `15f0287`.


## D-182: Merge detection is scoped to the revision family; non-winning revisions are Edit-locked

**Status**: Active · **Date**: 2026-08-31

**Context**: MERGE-1 (2026-07-28) computed `needs_merge` per `view_relative_path`: the current version is owner-authored (`saved_by == "auto"`) AND at least one prior version at that same path was human-authored. `manual_triage_required` mirrors it to SP, and the SP UI disables Submit-to-Carrier while any item in the milestone has it set (Pattern A per `[D-164]`).

That covers an owner resending under the **same** filename, where every version stacks on one path. It misses the resend-under-a-different-name case: `report_v2.xlsx` lands on its own view path, so a TPM edit on `report.xlsx` sits in a separate version history the rule never consults. Both paths look individually innocent — path B has one owner-authored version, path A's current version is the TPM's — so no flag fires, `manual_triage_required` stays False, Submit stays enabled, and once D-181's family-based selection is in place the owner's file ships while **the TPM's edit is silently dropped.** This is precisely the case `manual_triage_required` exists to catch, confirmed by the user 2026-08-30 ("this is the reason we introduced / used manual_triage_required — has to be fixed").

Sub-question: where does the visual marker go when a family spans two paths? And should a stale revision merely be flagged, or made un-editable?

**Decision**:
1. **Keep the MERGE-1 predicate verbatim; widen its inputs from path scope to family scope.** Family is `(delivery_item_id, doc_id_slug)`. `current` becomes the current version of the family's **winning** revision (the one D-181 would upload); `history` becomes the union of `saved_by` across every version of every path in the family.
2. **The marker renders on the path the TPM actually edited**, not on the winner — user rule 2026-08-30: "asterisk is on the version that TPM edited as they are familiar with what they edited and other versions are provided for their merge."
3. **New `TgFileEntry.is_superseded`** marks every non-winning revision of a family. The TG view badges it and withholds Edit by downgrading `open_mode` to `download` (the same mechanism D-152 uses for DRM-wrapped files), and `/browse/edit` enforces the same rule server-side via `storage.is_superseded_revision` — HTTP 409 plus a pointer to the current revision. User direction 2026-08-31: "TPM can edit only one version."
4. Families with a single path evaluate identically to before; paths with no resolvable family fall back to the original path-scoped predicate.

**Why**:
- **Widening scope rather than changing the predicate keeps the semantics reviewable.** The rule "owner landed on top of a human edit" was already agreed; only its notion of "where to look" was wrong. Single-path families are provably unchanged, which is what made the existing MERGE-1 tests a regression net.
- **The marker on the TPM-edited row matches how a TPM recognizes their own work.** The winner is the file they must merge INTO; the marked row is the content they must carry forward. Putting the marker on the winner would tell them something needs merging without showing what.
- **Locking rather than only flagging, because editing a stale revision produces work that provably cannot ship.** D-181 selects the family winner, so an edit to a non-winner is invisible to the carrier. The lock is enforced twice (UI withholds the link, route refuses) because a bookmarked token reaches the route directly — the same belt-and-suspenders shape D-152 uses.
- **The `/browse/edit` guard delegates to `list_files_in_tg` rather than re-deriving the family join**, so it cannot drift from what the TG view showed. It **fails OPEN** on any unresolvable path: a lookup glitch must never block a legitimate edit, and the UI is the primary control.
- **`manual_triage.py` needed no change**, which is a useful property rather than luck: it ORs `needs_merge` across every path whose version hashes resolve to an item, and the flagged path's v1 hash carries an association to that same item. The SP writeback and Submit gate therefore pick up family-scoped merges unmodified.
- **Validated end-to-end on live corp data**: v1 owner → v2 TPM → v3 owner set the flag and rendered the asterisk; a v4 TPM edit cleared it and re-enabled Submit.

**Consequences**:
- **A stale TPM edit now blocks the whole milestone's submission** until a TPM edits the winning revision. Intended — the alternative is shipping a file that silently lost their work.
- **`TgFileEntry` grows a field**, consumed by the TG-view template and the dashboard row dict.
- **New audit `action_type` `document_edit_blocked_superseded`**, with a humanized label in the History timeline.
- **Superseded state is about revision order, not merge state** — an older revision stays superseded after the merge is resolved. Correct, but means the badge persists on rows that no longer need action.
- **The `is_superseded` branch requires a multi-path family to exercise**, i.e. a resend under a different filename; a same-filename resend never produces it. Exercised on live data 2026-09-01 once a `_v2`-suffixed copy arrived.
- **Editing an older revision is refused, not redirected.** The 409 page links the download and tells the TPM to open the latest; it does not auto-navigate.
- 15 tests (`test_merge_family.py`), including the S4 case where a TPM edits a stale revision and the flag correctly does NOT clear.

**Anchors**: MERGE-1 (2026-07-28 predicate, unchanged); MTR-1 (`manual_triage_required` persistence + SP writeback); `[D-164]` (Pattern A — HILDA writes state, SP gates the button); D-152 (DRM Edit-withholding mechanism reused); D-180 (families this scopes to); D-181 (winner selection that makes a stale edit unshippable); commit `40bbeac`.


## D-183: Carrier sub-folders derive from archive provenance recorded at ingest, not from path shape

**Status**: Active · **Date**: 2026-08-31

**Context**: D-181 moved carrier upload to the view tree and derived the carrier sub-folder from the view path — everything after `view/<c>/<d>/<m>/<tg>/` minus the basename — on the assumption that leading segments came from inside an archive. True for email attachments. **Wrong for NSD**: `nsd2_poll` passes the SHARE-RELATIVE path as the attachment filename, so an ordinary file sitting in an NSD folder carries segments too. The live result was NSD tree structure leaking to the carrier: `Documentation/Device Compliance Entrance Criteria/2. DMDform (Done)/DMD_...xlsx`, where the user's spec 2026-08-31 is "non-zip file is directly uploaded at target_folder; if a zip file, the content of zip file (maintain folder/sub-folder recursive structure) at target_folder". `UPLOAD-SUBDIR-PLM-1`'s `ingest_source == CorporatePLM` gate had incidentally kept NSD flat before D-181 removed it.

**The path cannot distinguish the two cases.** NEST-1 prefixes an inner entry with its archive's filename only at depth >= 1, so a TOP-LEVEL zip containing `i am c/d.pdf` reaches the view tree as `i am c/d.pdf` — byte-identical in shape to an NSD folder. Provenance has to be recorded where it is known.

Options:
- **Option A (keep the folders)**: treat NSD structure as meaningful to the carrier. Rejected by the user.
- **Option B (flatten NSD-derived folders; preserve only archive-derived ones)**: needs a provenance marker.
- **Option C (re-narrow to `ingest_source == CorporatePLM`)**: restores pre-D-181 behaviour but leaves email/NSD zips flat, contradicting the spec that a zip's structure is preserved.

**Decision**: Option B, per user direction 2026-08-31 ("we need to fix this, no sub folders from NSD to go to carriers; flatten NSD-derived folders — only preserve subdirs when the document came from an archive").

`document_index.from_zip` and `source_zip_filename` already existed on the Pydantic model, the SQLAlchemy table and `storage/MODULE.md` as FR-72 — **but were never written by any ingest path**, always `False`. This wires them up: `_process_regular_attachment` and `_persist_routed_attachment` take `from_zip` / `source_zip_filename` (default `False`) and the index row persists them; `_process_archive_attachment` passes `from_zip=True` when it dispatches an extracted leaf, with the archive's own filename as `source_zip_filename`. Every other caller keeps the default, so standalone files — NSD tree, loose email attachment, PLM loose file — are `False`. `ItemUploadFile` surfaces `from_zip`, and `submit_to_carrier` computes the view sub-dir ONLY when it is set. The `UPLOAD-SUBDIR-PLM-1` internal-tree fallback is untouched and still CorporatePLM-gated.

**Why**:
- **Provenance is knowable only at ingest.** `_process_archive_attachment` is the one place that knows a byte stream came out of a container. Reconstructing it later from the path is impossible for the top-level-zip case, which is the common one.
- **Reusing FR-72's existing fields beats inventing a marker.** They were specified for exactly this ("ZIP ingest source flag" / "original ZIP filename when from_zip=true"), declared everywhere, and simply never populated — a latent gap this closes rather than a new concept.
- **Flat-by-default is the safer failure direction for existing data.** Pre-existing rows are all `False`, so previously-extracted archive contents will upload flat until re-ingested. The alternative — defaulting unknown provenance to "preserve" — would have kept leaking NSD folders to carriers. A missing sub-folder is visible and fixable; an unexpected folder in a carrier deliverable is a submission defect.
- **Gating in `submit_to_carrier` rather than in the resolver** keeps `list_upload_files_for_item` a pure "what should ship" query and leaves the carrier-shaped path composition in the task that talks to the carrier.

**Consequences**:
- **FR-72's `from_zip` / `source_zip_filename` are now live data** after being inert since the schema was written. Anything that previously assumed they were meaningless is wrong going forward.
- **Backfill is only partially possible.** `source_zip_filename` is derivable from the `.zip` segment in `local_nsd_path` for NESTED archives, but not for top-level ones — the same ambiguity that made the flag necessary. Tracked in STATUS Flags.
- **Archive contents ingested before 2026-08-31 upload flat.** If a real submission depends on an already-ingested zip's structure, it needs re-ingesting.
- **NSD folder names never reach the carrier**, even when a TPM organizes the share into meaningful subfolders. Deliberate: the carrier's folder taxonomy is `target_folder`, not the share's.
- **A regression introduced and fixed within 24 hours**, which is itself the argument for the two-axis / provenance separation being non-obvious.
- 5 tests, including a `submit_to_carrier` test asserting an NSD-foldered file uploads flat.

**Anchors**: FR-72 (ZIP ingest flags, finally implemented); D-181 (the sub-folder derivation this corrects); `UPLOAD-SUBDIR-PLM-1` (2026-08-27, the CorporatePLM gate that governs the internal-tree fallback and previously masked this); D-155 / NEST-1 (archive-as-container ingest + the depth-dependent path prefixing that makes shape ambiguous); `[D-054]` (individual files to the carrier, never zips); commit `9971e98`.


## D-184: PLM eligibility is gated on `tracking_modality` alone; the code-side TG allowlist is removed

**Status**: Active · **Date**: 2026-08-31

**Context**: `plm_poll._filter_eligible_items` gated on BOTH a hardcoded `_PLM_ELIGIBLE_TG_NAMES = frozenset({"mql-fit", "mno-solution"})` AND `tracking_modality` containing `CorporatePLM`. The TG set dates from PLM-3; the modality gate arrived later in PLM-7 as an ADDITIONAL strict gate. By 2026-08-31 the two had drifted: a live GPS item on MMK/SM-DEVICEB/P1 carried `tracking_modality=["CorporatePLM"]` in `template.yaml` and had a populated `owner_corp_id`, but was silently filtered out by the TG set — so it never got a ticket and its `plm_id` stayed NULL indefinitely.

This was the second time the same set caused a silent live outage: PLMCASE-1 (2026-08-18) had to make it case-insensitive after 44 P1 `MNO-Solution` items went invisible because the frozenset held `MNO-SOLUTION` and the SP data said `MNO-Solution`.

Options:
- **Option A (add `"gps"` to the frozenset)**: one line. Repeats the problem on the next TG onboarded.
- **Option B (remove the TG allowlist; gate on `tracking_modality` alone)**: `template.yaml` becomes the single source of truth.
- **Option C (move the allowlist into config)**: keeps a second gate but out of source.

**Decision**: Option B, chosen by the user 2026-08-31 after the options were laid out with the supporting data.

The frozenset is deleted. `_filter_eligible_items` now requires: `CorporatePLM` in `tracking_modality`, `device_id` matches scope, non-empty `tg_name` (the `[D-035]` one-ticket-per-TG grouping key), and a non-terminal `delivery_state`. `_group_by_tg` still lowercases the grouping key so casing variants collapse into ONE ticket.

**Why**:
- **Behaviour-neutral at the time of the change, which made it the cheap option rather than the risky one.** Live data showed only three TGs carrying `CorporatePLM` — GPS, MNO-Solution, MQL-FIT — so Option B produced exactly the same eligible set as Option A on the day. The difference is that onboarding a TG no longer needs a code edit, a deploy, and a worker restart.
- **`tracking_modality` IS the declaration of intent.** It is a TPM-facing SP Choice column (per `[D-037]`) and `template.yaml` is already authoritative for structural DeliveryItem fields per `[D-141]`. A code-side list shadowing TPM-authored data is the recurring failure shape — twice in two weeks, both silent.
- **The TG gate was vestigial.** It predated the modality gate, which is documented as strict with no fallback. Keeping both meant maintaining two sources of truth for one question.
- **Removing a brake was an explicit, recorded trade-off**, not an oversight — see Consequences.

**Consequences**:
- **Setting `CorporatePLM` in `template.yaml` is now sufficient to start creating real PLM tickets for a TG**, with no second brake in code. Accepted by the user 2026-08-31 on the grounds that `template.yaml` is edited deliberately. If a TG is configured before it is ready, tickets get created.
- **`tg_name` must be non-empty** to be eligible — it is the grouping key, and an empty key would be dropped by `_group_by_tg` anyway. Filtering earlier keeps the two functions' contracts aligned.
- **GPS creating a ticket is the first end-to-end exercise of the change.** A `create_plm_ticket` failure there surfaces as `tickets_create_failed` rather than the previous silent invisibility, which is itself an improvement.
- **One test rewritten rather than deleted**: `test_no_eligible_items_skips_group_processing` used `tg_name="HW PL"` to force zero eligible items, which the modality-only gate makes eligible. It now opts out via modality, and a new test asserts the GPS case (any TG with `CorporatePLM` is eligible).
- **The PLMCASE-1 case-insensitivity fix is now only relevant to `_group_by_tg`**, not to eligibility. Its historical note is preserved in a comment at the deleted set's location so the reason the set existed — and why removing it is safe — is not lost.

**Anchors**: PLM-3 (original TG-scoped eligibility); PLM-7 (modality gate that superseded it); PLMCASE-1 (2026-08-18, the first silent outage from this set); `[D-037]` (multi-value `tracking_modality`); `[D-141]` (template-authoritative structural fields); `[D-035]` (one PLM ticket per TG); D-169 (PLM poll architecture); commit `a286505`.


## D-185: Cross-milestone submission is read-through, never a copy

**Status**: Active · **Date**: 2026-09-01

**Context**: Deliverables collected against DRR work-items are the same artifacts the P1 milestone must submit to the carrier. DRR never uploads on its own path — every mapped DRR item is `no_customer_upload: true` / `target_folder: null` in `template.yaml`, verified across all 24 pairs — so without a mechanism those documents are collected and then never delivered. The user supplied a 24-pair `item_no -> item_no` mapping (DRR -> P1) spanning 8 TGs.

A hard constraint shaped the design: **`document_index.file_hash` is the sole primary key**, and the row carries `milestone_id`. Identical bytes therefore CANNOT be indexed under two milestones. Options considered:
- **Option A (new `document_index` rows per milestone)**: blocked outright by the PK.
- **Option B (association-level fan-out — add `document_item_association` rows pointing the P1 item at the DRR document)**: possible, since that PK is `(file_hash, delivery_item_id)`. Costs a write per document per mapping and leaves `document_index.milestone_id` saying DRR anyway.
- **Option C (read-through — resolve the source item's documents at read time, create nothing)**: user direction 2026-08-31, stated as "do not need to copy… there is no need to fire copy; at submit_to_carrier task of P1 milestone, files of DRR are also uploaded".

**Decision**: Option C. Three-piece implementation:
1. **Mapping config** at `customizations/template_schemas/<customer>/milestone_item_mapping.yaml`, per carrier, `mappings: [{source_milestone, target_milestone, pairs: {src: tgt}}]`. Pure int -> int; trailing comments are free text. Cached loader mirroring `template_lookup`, eager-loaded at bootstrap, both lookup directions precomputed.
2. **Resolver** `storage.list_migrated_upload_files_for_item(target_item_id)` — target row -> reverse mapping -> source row -> delegate to `list_upload_files_for_item` (D-181). Rows resolved by natural scope `(customer, device, milestone, item_no)`, not by composing or parsing the `[D-091]` item_id string.
3. **Submit integration** — own files + migrated files merged per item, uploaded under the TARGET item's `target_folder`. The source item is never transitioned.

**Why**:
- **A document should exist once, indexed under the milestone it actually arrived in.** Option B would have made "which milestone does this document belong to?" ambiguous — the index row says DRR while an association says P1 — for no gain, since the upload path needs a resolution step either way.
- **Delegating to D-181's resolver rather than reimplementing** means migrated documents inherit revision-family collapse, current-version resolution, waiver exclusion, and the flat-vs-archive decision automatically. Tests pin each of those THROUGH the mapping so a future change to the base resolver cannot silently diverge for migrated files.
- **Natural-scope lookup instead of item_id composition** because device ids contain hyphens (`SM-DEVICEB`), so the `{customer}-{device}-{milestone}-{item_no}` composite cannot be parsed back apart; composing it forward would bake the id scheme into a third call site.
- **Misses are silent by design.** User's contract: "there is no guarantee of file existence in DRR, just because mapper asks for it, meaning whatever mapped file available from DRR, those are uploaded." An unmapped item, or a mapped item whose document has not arrived, is the normal state for most of a milestone's life. The one WARN is a mapping naming a work-item the device does not have — that means template and mapping have drifted.
- **No content-level dedup between own and migrated files.** User 2026-09-01: a document received in the target milestone always differs in hash from the source milestone's versions, so both legitimately ship. The only guard is against the same PATH appearing twice.
- **Validation guards the shipped mapping file, not just the loader.** All 24 pairs are cross-checked against `template.yaml` in CI, so a renumbered template fails there rather than at a carrier submission.

**Consequences**:
- **No rows are created in `document_index`, `document_item_association` or `document_version`** by the migration. Asserted by a test comparing row counts across a resolve.
- **`document_index.milestone_id` for a migrated document stays DRR.** Any future query that scopes documents by that column will not see it as a P1 document; only the item-keyed paths do.
- **Migration cannot break a submission that would otherwise succeed.** Absent resolver is a silent no-op; a raising resolver is caught, warned, and the item proceeds on its own documents.
- **New audit `action_type` `submit_to_carrier_migrated_file_ok`**, and every migrated audit row carries `migrated_from_milestone` / `migrated_from_item_no`, so a carrier folder holding a borrowed document is traceable.
- **New stats are a breakdown, not an addend**: `files_uploaded_migrated` / `files_failed_migrated` are subsets of `files_uploaded` / `files_failed`. A broken mapping reads as "migrated dropped to 0" rather than hiding inside a smaller total.
- **Two mapped pairs cross `item_type`** (7 -> 29 and 64 -> 18, `compliance_certification_release_notes` -> `test_tech_waiver_report`). Harmless precisely BECAUSE no association is created — FR-86's alignment invariant is never re-evaluated and the document keeps its source-side `doc_type`. Under Option B this would have needed an alignment decision.
- **Two mapped targets share a `target_folder`** (P1 #14 and #20 both -> `RF Parametric Data/Documentation/FCC Package`), so documents from DRR #69 and #67 land in one folder; with flat uploads per D-183, identically-named files there would collide. Recorded in the mapping file header.
- **Source-milestone merge state does not gate target submission yet.** SP gates Submit per-milestone on `manual_triage_required`, which sets on the DRR row while SP checks P1 rows — so an unresolved merge on a DRR document would not block a P1 submission. User decided 2026-09-01 to gate it; scoped as DRRP1-1 chunk 5.
- **Target-milestone visibility of source documents is a separate decision** — user chose item-scoped (the P1 item's document section, read-only, labelled) over TG-view merging, because the mapping is item -> item while the TG view is TG-scoped, and TG alignment across all 24 pairs is a coincidence in the data rather than a guarantee. Scoped as DRRP1-1 chunk 4.
- 59 tests across the three chunks (45 loader incl. a shipped-file guard, 14 resolver, 8 submit wiring).

**Anchors**: `[D-091]` (item_id composition + `item_no` uniqueness per customer/device/milestone); D-181 (upload resolver this delegates to); D-183 (flat-vs-archive sub-folder rule migrated files inherit); `[D-141]` (template-authoritative structural fields — the mapping lives beside `template.yaml` for the same reason); `[D-035]` / FR-86 (item_type alignment, deliberately not re-evaluated); `[D-054]` (individual-file carrier upload); commits `f5b4f9a`, `cf9795c`, `d7e58cb`.
## D-186: Carrier-destination resolution is shared, never duplicated

**Status**: Active · **Date**: 2026-09-06

**Context**: The rule for where a file lands on the carrier --
`<target_folder>[/<subdir>]/<filename>` -- lived inline in `submit_to_carrier`'s
per-item loop, private to a Celery task. Three consumers then needed the same
answer: the uploader, the TG document view (which must show a TPM where a
document will land), and a new milestone-wide submission preview.

This exact failure had already happened one layer up. `DOCTYPE-MISALIGN-UI-1`
(2026-09-03) found the TG view rendering a document as classified while
`list_upload_files_for_item` was silently dropping it from the submission,
because the two read different predicates. A preview that computes a different
path from the uploader is worse than no preview: it manufactures confidence.

**Decision**: Extract the destination rule into `core/src/storage/upload_plan.py`
and have every consumer call it. Two seams were created in the process:

- `carrier_subdir(relative_path, is_view, from_zip, ingest_source)` -- the
  view-vs-internal branch that had been inline, including the `UPLOAD-FLAT-1`
  `from_zip` gate.
- `effective_target_dir(target_folder, subdir)` -- composition isolated so that
  exactly one place substitutes the folder (used later by D-187).

`submit_to_carrier` re-exports the moved helpers under their original private
names so existing references and tests keep working.

**Why**:
- **Fidelity is the whole requirement of a preview.** The only way display and
  delivery cannot drift is for them to be the same code. Anything weaker is a
  convention that decays.
- **The extraction is behaviour-neutral**, verified by the suite before and
  after -- so the risk sits entirely in the refactor, not in the new features
  that build on it.
- **The module's own imports are stdlib-only**, so the dashboard can use it
  without pulling Celery into the API container. An AST test keeps them that
  way, which also leaves the module relocatable later.

**Consequences**:
- Any future change to carrier path shape has ONE edit site; forgetting a
  caller is no longer possible.
- Importing it still runs `core.src.storage.__init__` and therefore SQLAlchemy.
  That was already true of the dashboard via `document_view_ops`, so only
  Celery was a genuinely new dependency; the test asserts that specifically.
- A structural fact was confirmed and pinned while doing this: the view tree is
  `view/<customer>/<device>/<milestone>/<tg>/<parts>` -- TG-scoped with NO item
  segment. Moving a document between work-items inside one TG therefore leaves
  the view path, and the archive-derived sub-folder, unchanged. D-187 depends
  on this.
- 35 tests.

**Anchors**: D-181 (upload resolver), D-183 (archive-provenance sub-folders),
`UPLOAD-FLAT-1`, `DOCTYPE-MISALIGN-UI-1` (the drift this prevents).


## D-187: A TPM redirects a document by overriding its carrier folder, not by re-assigning it

**Status**: Active · **Date**: 2026-09-06

**Context**: Documents sometimes route to a work-item whose `target_folder` is
not where the carrier expects them -- most often via a TG-level `['default']`
catch-all. The TPM's concern, stated directly (user 2026-09-06): *"TPM is
concerned about where the doc is landing in google drive and not about to what
item the doc is routed to."*

Re-assignment already exists (FR-87) but is rejected by the storage layer for
`CLASSIFIED` documents (`STR-E009`), which is precisely the population at risk.

Three options were considered:
- **(a) Edit the work item's `target_folder`** -- rejected: affects every
  document on the item, and template re-import overwrites it.
- **(b) Per-document override of the folder** -- one nullable column, consulted
  at the single `effective_target_dir` seam from D-186.
- **(c) Re-assign the document to a different work item** -- the "correct
  model" reading, and what the existing machinery does.

**Decision**: Option (b). A nullable
`document_item_association.upload_target_folder_override` (VARCHAR 512, alembic
`0005`), preferred over the item's `target_folder` in both `submit_to_carrier`
and the preview. The archive-derived sub-folder still rides underneath, so zip
structure survives a redirect. The picker offers only folders already declared
by a work item in the SAME TG.

**Why**:
- **A folder choice carries no item information.** P1 #14 and #20 both target
  `RF Parametric Data/Documentation/FCC Package`. When a TPM picks that folder,
  nothing in the choice says which item was meant -- so forcing it into an item
  assignment invents an intent the TPM never expressed.
- **Re-assignment drags in four unrelated risks**: it splits the revision
  family unless every member moves atomically; it moves internal-tree files; it
  changes which P1 item a DRR document migrates to, because
  `milestone_item_mapping` maps item -> item; and it requires relaxing the
  `STR-E009` A->B->C ordering guard.
- **The offered set prevents the failure mode this approach is usually accused
  of.** The initial objection (2026-09-05) was that an override papers over a
  template gap. It cannot: the TPM can only select a folder some work item
  already declares. A destination that ought to exist and does not still has to
  be added to `template.yaml`.
- **Nothing moves and no state changes**, so PM approval in SP is unaffected --
  confirmed acceptable by user 2026-09-06: a folder change after approval is
  expected behaviour.

**Consequences**:
- The Drive layout is no longer derivable from `template.yaml` alone; the
  override column is a second input. Every set/clear writes an audit row
  (`set_upload_folder_override`) and logs at WARNING, and `submit_to_carrier`
  logs whenever an override is in effect.
- **alembic `0005` must run on the corp Postgres BEFORE the worker picks up
  this code**, or association reads fail on a missing column.
- The picker is hidden on rows already excluded from upload (waiver, staged,
  superseded) -- offering a folder there would imply picking one fixes the
  exclusion, which it does not.
- An override also rescues an item that has no `target_folder` of its own,
  which is a cheaper answer than D-191's route for a genuine one-off.
- The POST re-validates server-side rather than trusting the form; accepting an
  arbitrary string would deliver documents to a path no work item declares.

**Anchors**: D-186 (the single seam it substitutes at), D-188 (its scoping),
`[D-153]` (a document lives under exactly one TG folder), FR-87 / `STR-E009`
(the re-assignment path not taken).


## D-188: TPM document-level intent is scoped to the revision family, not the file

**Status**: Active · **Date**: 2026-09-06

**Context**: While designing D-187 the same hazard appeared from two
directions. Re-assigning one `file_hash` would leave other revisions of the
same `doc_id_slug` on the original item -- and since upload picks one winner
per family, the ORIGINAL item would then upload a superseded revision. Keying
a folder override on one `file_hash` has the mirror failure: the TPM redirects
rev1, the owner sends rev2, and rev2 silently uploads to the original folder,
because the new revision carries no override.

**Decision**: Any TPM-expressed intent about a document applies to every member
of its `doc_id_slug` family within the milestone. `set_upload_folder_override`
and `clear_upload_folder_override` resolve the family first and write to all of
its associations. A document with no index row, or no slug, is its own
single-member family -- matching how `list_upload_files_for_item` groups.

**Why**:
- **The TPM is expressing intent about a DOCUMENT, and a document is a family.**
  The UI shows one row per view path, so a resent document appears as two rows
  (one badged `superseded`); acting on the row the TPM happens to be looking at
  would silently apply to a fraction of the document.
- **Both failure directions are silent.** Neither the stale-revision upload nor
  the un-inherited override produces an error; the wrong bytes or the wrong
  folder simply reach the carrier.
- **It generalises.** Stated as a rule rather than an implementation detail so
  that any future per-document control (pin, suppress, re-route) starts from
  the right unit.

**Consequences**:
- A revision arriving AFTER an override is set does not inherit it until the
  override is re-applied -- the write is at set-time, not read-time. Recorded
  as a known limitation; a read-time resolution by slug would remove it.
- The family lookup is one extra query per set/clear, on a TPM-initiated path
  where that cost is irrelevant.
- Two tests pin the property directly, because it is invisible in normal use
  and would regress unnoticed.

**Anchors**: D-180 (revision-family identity via `doc_id_slug`), D-181
(one file per family at upload), D-187 (the first control scoped this way).


## D-189: The NSD2 carrier folder is located by search, and the gate is an allowlist

**Status**: Active · **Date**: 2026-09-01/06

**Context**: The requirement is that only documents under a carrier's folder
(`CARRIER` for MMK) reach Google Drive. The walker was anchored at the resolved
device folder and filtered with a DENYLIST of substrings
(`MMK_EXCLUDED_FOLDER_SUBSTRINGS`). Three defects followed, each silent:

- Loose files at the device-folder root were ingested -- for `S948U (M3)` a
  stray workbook and a 338 MB `CARRIER M3 HW Deliverables.zip` that the archive
  extractor would then fan out.
- A denylist blocks only what someone enumerated. `STG` was excluded; `ATT`,
  `TMO`, `Sprint`, `USC` were not.
- Substring matching over-matched: `CHA` (Charter) pruned `Charging`,
  `Mechanical`, `Exchange`, `Chart` -- with their whole subtrees -- and logged
  the prune at INFO, which the containers drop at WARNING.

Three real folder layouts exist under the same share, discovered one at a time:
`CARRIER/` at depth 0 (`S948U`), `Deliverable/CARRIER/` at depth 1 (`F776U`), and no
carrier folder at all (`S731U`, numbered deliverable folders directly under the
device).

**Decision**: Per-carrier allowlist (`CARRIER_ALLOWED_ROOT_FOLDERS`, MMK ->
`("CARRIER", "CARRIER")`) with the folder located by breadth-first search:

- Stop at the SHALLOWEST level containing an allowlisted folder, so a genuine
  depth-0 `CARRIER/` always beats anything deeper.
- Never descend into a denylisted folder, so `STG/CARRIER/` can never become an
  anchor.
- The walk is SEEDED from the anchors, so everything outside them is
  unreachable rather than filtered.
- Capped at `NSD2_ANCHOR_SEARCH_MAX_DEPTH = 3` (architect 2026-09-06).
- No anchor found -> fall back to the whole device folder, logged as
  `NONCONFORMING LAYOUT` naming the folder.
- Single-word denylist needles now match a whole TOKEN, not a substring.

**Why**:
- **A denylist cannot express "only CARRIER".** It is open by default, and every
  new sibling folder is a silent leak until someone notices.
- **Anchoring, not filtering.** Making everything outside the anchor
  unreachable removes the class of bug where an unenumerated sibling passes.
- **The cap is not a safety mechanism and must not be mistaken for one.**
  Exceeding it downgrades to ingesting the whole device folder, so
  `find_carrier_anchors` WARNs when it truncates with folders unexamined.
- **The fallback is loud on purpose.** It is a guess, and the HW PL TG needs a
  concrete worklist of folders to normalise rather than an abstract
  instruction. This is step 1 of standardising on "a carrier folder must
  exist"; when those log lines reach zero the fallback can become
  refuse-and-alert.
- **Token matching over substring** because the false positives were real
  HW-deliverable folder names and the loss was invisible.

**Consequences**:
- Two of my own corrections are in the history: gating unconditionally broke
  flat device folders (nothing ingested from `S731U`), and a depth-0-only probe
  missed `Deliverable/CARRIER`. Both were live for under an hour.
- Every skip now logs at WARNING; the summary line carries the anchors and the
  mode (`anchored` / `whole-device-folder`).
- "A foreign-carrier folder was seen but no CARRIER" was deliberately NOT made
  grounds to ingest nothing -- a stray `DISH Config/` deep inside a deliverable
  folder does not make a device folder carrier-partitioned, and silently
  ingesting nothing is the worse failure. The flag survives as log context.
- For MMK the denylist is now unreachable in production (it is the only
  `EXCLUSION_CARRIERS` member and now has an allowlist); kept intact because
  `is_excluded_folder_name` is still exported, tested, and used by the anchor
  search.
- Adding a carrier to NSD2 ingestion requires a `CARRIER_ALLOWED_ROOT_FOLDERS`
  entry or it silently gets the old permissive behaviour.

**Anchors**: `[D-153]` (a document lives under exactly one TG folder), NSD2-12
(`tracking_modality` gate), `UPLOAD-FLAT-1` (the sibling rule for sub-folders).


## D-190: doc_type multi-match resolves by explicit precedence, waiver first

**Status**: Active · **Date**: 2026-09-02/04

**Context**: A live DRR ingest stored three files named
`SM-DEVICEF Waiver Request_*.ppt` as `compliance_certification_release_notes` --
aligned and CLASSIFIED at rev1, not staged, with no TPM signal. Filing a waiver
as a delivered compliance document is the worst available outcome, because
waivers are never uploaded on the P1 submit path: mislabelling one is how it
gets SENT to the carrier.

Three independent defects combined:
- `.ppt` was absent from every extension group, and 32 compliance rules carried
  `pptxi` -- a typo accepting a non-existent extension while rejecting real
  `.pptx`. The waivers matched NO rule.
- `_classify_doc_type` returned `UNRESOLVED` on ANY multi-match.
- `UNRESOLVED` is not neutral: it hands the document to
  `AUTO-CLASSIFY-RELNOTES-1`, which classifies from the ROUTED ITEM's
  `item_type` rather than the document's own name.

Separately, 21 regexes were written as `word1*word2` intending "both words with
anything between". In regex a bare `*` means zero-or-more of the preceding
character, so those patterns were near-dead.

**Decision**:
- **Precedence on multi-match**: `waiver` -> `compliance` -> `tech_report` ->
  `test_report`. Order lives in code (`_DOC_TYPE_PRECEDENCE`), not in YAML key
  order.
- **One canonical extension set** applied to every pattern at load time
  (`{pdf,doc,docx,xls,xlsx,xlsm,ppt,pptx,html,htm}`), rather than editing 123
  YAML lines.
- **Waiver veto**: a filename carrying the template's `waiver` marker is never
  auto-promoted to another doc_type.
- **`word*word` -> `word.*word`** across all 21 patterns.

**Why**:
- **Waiver is first because it is the only doc_type with an outward-facing
  consequence.** Every other misclassification is recoverable inside HILDA.
- **Returning UNRESOLVED on multi-match was actively harmful, not merely
  unhelpful**, because of what consumes UNRESOLVED downstream.
- **Order in code, not config**, so re-sorting `doc_type_filename_rules.yaml`
  cannot silently change classification behaviour.
- **Normalising at load keeps the config readable** and makes the extension set
  impossible to skew again -- the three divergent groups were the actual bug.
- **Both fixes were required for the live case.** With extensions fixed, two of
  the three `.ppt` waivers match BOTH the waiver rule and a compliance rule, so
  the extension fix alone would still have mis-filed them.

**Consequences**:
- 14 of 20 representative filenames changed classification, every one from
  `UNRESOLVED` to a concrete type. `[Miracle] VoLTE ODR.pdf` moved from
  `test_report` to `compliance`, which now ALIGNS with all three MNO-UX items
  it can route to, so it lands CLASSIFIED instead of staging silently.
- **Re-classification applies to newly ingested documents only.** The three
  `.ppt` waivers already in Postgres remain mis-filed and need a TPM
  Reclassify.
- `pptxi` is gone, so 32 compliance rules now accept real `.pptx` and reject
  the typo -- a behaviour change for any file that relied on the old spelling
  (none observed).
- A doc_type present in the YAML but absent from `_DOC_TYPE_PRECEDENCE` falls
  through and stages loudly rather than being guessed; a test asserts both
  rules files are fully covered.
- The keyword fallback records `FilenameFallbackKeyword` rather than
  `FilenameRegex`, so guessed classifications stay countable and the
  UNRESOLVED -> STAGED -> TPM-reclassify feedback loop is not destroyed.

**Anchors**: FR-85 (2-step ladder), FR-86 (alignment invariant),
`AUTO-CLASSIFY-RELNOTES-1` (the consumer that made UNRESOLVED dangerous),
`DOCTYPE-MISALIGN-UI-1` (surfacing what still misaligns).


## D-191: A DRR-only deliverable gets a P1 upload-only target, not a DRR target_folder

**Status**: Active · **Date**: 2026-09-04

**Context**: DRR #79 collects the 4G/5G DOU test plan (tg `MQL PC`) and has no
P1 counterpart, but the carrier expects it under
`Feature Test Results/Battery`. Two options: add a `target_folder` to the DRR
item, or create a P1 work item and map to it.

The DRR option looks viable from the code: `submit_to_carrier` has NO milestone
gate -- it skips only on `no_customer_upload` or a null `target_folder`. It
fails for an external reason: **SP does not expose Submit-to-Carrier for the
DRR milestone**, so `milestone_submission_triggered_at` is never set on the DRR
Milestones row, the trigger rule never fires for DRR, and the task never runs
with `milestone_id=DRR`.

**Decision**: New P1 #114, upload-only, plus mapping pair `79 -> 114`.
`item_description: null` and `tracking_modality: []` so routing never selects
it and NSD/PLM eligibility excludes it; `tg_name: "MQL PC"` matching DRR #79.

**Why**:
- Setting `target_folder` on DRR #79 would have written two fields that nothing
  ever reads -- a silent no-op that looks correct in the template.
- The mapping requires a target `item_no`; with no P1 counterpart there was
  nothing to map to.
- Creating the item keeps the delivery path identical to the other 24 pairs
  rather than introducing a second mechanism.

**Consequences**:
- **P1 #114 must be walked to `ReadyForSubmission` by a TPM in SP or DRR #79's
  documents are never delivered.** It receives no documents of its own, so
  nothing advances it automatically -- unlike the other 24 targets, which are
  real items that advance on their own with DRR files riding along. The failure
  is silent: the pair is valid, the folder is right, and nothing uploads.
  Recorded in both `template.yaml` and `milestone_item_mapping.yaml`.
- The mapping is now 25 pairs. Re-verification found NO pair crosses
  `item_type` any more (P1 #29 and #18 are both compliance now), correcting the
  header's 2026-09-01 claim.
- `MQL PC` contains a space, not a hyphen -- both sides must match or the
  mapping's tg_name check fails.

**Anchors**: D-185 (read-through migration), `[D-091]` (item_no uniqueness),
`[D-035]`.


## D-192: inferred_tg_name is populated channel-agnostically, and the unrouted path follows it

**Status**: Active · **Date**: 2026-09-03

**Context**: The manual-route dropdown offered every non-Confirmation,
non-Default item in the milestone, so a document received for one TG could be
routed to another -- against `[D-153]`, which says a document lives under
exactly one TG folder. Scoping it needed a per-document TG, and there was none:
the router's `tg_resolver` slot was wired to `None` for EVERY channel, so
`document_index.inferred_tg_name` had never been populated at ingest despite
the column, FR-78 and `[D-060]` all existing for it.

**Decision**: `sole_tg_resolver` -- return the single distinct `tg_name` across
the candidate set, else `None` -- wired into `_build_ph1_router`. The
manual-route dropdown resolves candidates per row, scoped to that TG, falling
back to the milestone-wide list when no TG is resolvable.

**Why**:
- **No per-channel plumbing is needed** because the candidate set is already
  TG-scoped upstream where it matters: `_filter_hw_pl_nsd2_items` gates NSD to
  `HW PL`, and PLM ingest runs inside `_process_tg_group`, one TG per ticket
  per `[D-035]`.
- **Email legitimately returns `None`** while one TPM owns items across many
  TGs -- the honest answer, and it produces exactly the milestone-wide list
  that case needs.
- **The fallback is deliberate**: an empty dropdown would leave a TPM unable to
  route at all, which is worse than a wide one.

**Consequences**:
- **On-disk layout changed for newly unrouted files.**
  `NSDPath.internal_default_workitem` takes `inferred_tg_name or "_unknown_tg"`,
  so NSD/PLM orphans are now written under `<TG>/_unrouted/` instead of
  `_unknown_tg/_unrouted/`. Existing files stay where they are; the share shows
  both shapes for a while. This is what FR-78 / `[D-060]` always specified --
  the placeholder was meant to be the exception.
- **A coupled fix was mandatory.** `route_unrouted_to_item` HARDCODED
  `_unknown_tg` when computing the source path to move from. Populating
  `inferred_tg_name` without that fix would have made every newly ingested
  unrouted file unroutable -- the move would read a path that does not exist.
  It now reads `doc.inferred_tg_name or "_unknown_tg"`, so both vintages
  resolve; tests pin each.
- Existing unrouted rows keep NULL, so their dropdowns stay milestone-wide.
  Scoping the backlog would need a one-off backfill.
- Unrouted documents are still surfaced on the single scope-level
  `/_unknownTG/` page; only the dropdown contents changed.

**Anchors**: `[D-153]` (one TG per document), `[D-060]` / FR-78
(`inferred_tg_name` and the unrouted path), `[D-035]` (one PLM ticket per TG).

## D-193: A cross-milestone target may be fed by several sources

**Status**: Active — **Date**: 2026-09-06

**Context**: `milestone_item_mapping` refused a second source claiming one
target, on the reasoning that "merging two document sets onto one work-item is
never the intent". MMK then needed exactly that: MNO-IOT has two DRR
work-items — #35 (Unified/Custom, and the TG default) and #60 (POA) — and only
ONE P1 item, #23, whose `target_folder` is `IOT Results/IT IOT`. Adding
`60: 23` produced a startup WARN and a mapping with 25 pairs instead of 26.
DRR #60's documents were collected against a real work-item and then never
uploaded, which is precisely the failure the mapping exists to prevent, and the
only signal was one line at bootstrap.

**Decision**: Fan-in is legitimate. `MappingBlock.reverse` maps a target to a
LIST of sources in mapping-file order, and `get_source_item_nos` /
`resolve_migration_sources` return every contributor. The forward direction
stays single-valued: one source item cannot deliver to two places, so a
repeated source is still dropped.

**Alternatives rejected**:
- **Fold POA into DRR #35** (add a `["POA"]` tag-set, delete #60). Config-only,
  no code change, but the TG loses POA as a separately tracked DRR work-item in
  SP and the DRR report. The mapping should follow the TG's tracking model, not
  dictate it.
- **A second P1 item to receive #60.** Keeps both DRR items, no code change,
  but manufactures a P1 work-item that collects nothing purely to satisfy a
  loader constraint — and, like #114 per `[D-191]`, a TPM would have to walk it
  to `ReadyForSubmission` by hand or nothing uploads. A config workaround for a
  code rule that was simply too strict.

**Consequences**:
- **Contributions collide, and nothing downstream disambiguates.** Uploads are
  flat per `UPLOAD-FLAT-1`, so if DRR #35 and #60 both offer `report.pdf` they
  resolve to one carrier path and the later write wins. Revision-family
  collapsing is per source item, so both survive that far.
  `list_migrated_upload_files_for_item` WARNs by filename when more than one
  source contributes — the only place both contributions are visible at once.
  `UPLOAD-BUNDLE-1` disambiguates in the preview zip; the carrier upload does
  not. Same exposure as P1 #14/#20 sharing `RF Parametric Data/Documentation/
  FCC Package`.
- **The loader still WARNs on fan-in**, now as information rather than refusal,
  because a fan-in that was a typo and a fan-in that was intended look
  identical in the file.
- **One absent contributor no longer suppresses the others.** A mapped source
  item the device does not have is skipped with a WARN; the remaining sources
  still deliver.
- `get_source_item_no` and `resolve_migration_source` survive as
  first-contributor helpers for callers that only ask WHETHER a target is
  mapped. Anything that collects documents must use the plural form — taking
  the first would reintroduce the bug this reverses.

**Anchors**: `[D-091]` (`item_no` unique per customer/device/milestone),
`[D-191]` (upload-only targets), `UPLOAD-FLAT-1` (flat carrier upload).

## D-194: HILDA is served under /hilda/*, and nginx strips the prefix

**Status**: Active — **Date**: 2026-09-08

**Context**: TPMs on other corp networks cannot reach the HILDA box directly;
a corp reverse proxy (Caddy, `<corp-proxy-host>:<port>`) fronts it and routes by
path prefix. Every TPM-facing URL had to move under a single `/hilda/*`
prefix so one proxy rule covers the whole surface.

**Decision**: nginx serves `location /hilda/` with `proxy_pass
http://hilda_api/` — the trailing slash STRIPS the prefix — so hilda-api
still receives and routes unprefixed paths. Only what the app EMITS carries
the prefix: hrefs, form actions, redirect targets, `download_url` payloads,
and the emailed feedback link. One definition in
`core/src/dashboard/url_prefix.py`, driven by `DashboardConfig.url_prefix`
(default `/hilda`, env `HILDA_DASHBOARD_URL_PREFIX`, set in the bind-mounted
`config/dashboard.json` so ops can change it without a rebuild). `""` serves
at the root again.

**Why not prefix the route declarations**: 154 test call sites drive the app
directly at bare paths. Prefixing the routes would have required rewriting
all of them, and those tests would then no longer exercise the paths nginx
actually forwards in production. The asymmetry is load-bearing, and
`test_url_prefix.py` pins it — including an assertion that the app must NOT
serve `/hilda/...` itself, so nobody "fixes" the asymmetry and silently
breaks the strip.

**Scope**: app routes only. `/wopi/` and `/office/` stay at the root — WOPI
is a JWT-signed OnlyOffice back-channel the TPM never types, and the
OnlyOffice `sub_filter` prefix rewriting is fragile enough without a second
layer. `/healthz` stays because the compose healthcheck curls
`hilda-api:8080` directly and never traverses nginx.

**Consequences**:
- Legacy paths 301 to `/hilda$request_uri`. `absolute_redirect off` keeps
  Location RELATIVE, so the browser reuses the Caddy hostname rather than a
  baked-in box IP. Verified: Caddy forwards the prefix intact, so no
  redirect loop.
- Emitted-URL drift is the failure mode, and it is silent: an unprefixed
  href sends the browser outside `/hilda/`, where nginx proxies nothing.
- Three absolute origins were stale and pointed at the box, unreachable from
  behind the firewall. `reverse_proxy_origin` and
  `list_documents_url_prefix` (x200) now name the Caddy origin.
  **`onlyoffice_public_url` still does not** — the OnlyOffice editor is
  expected to fail for off-network TPMs until it does, and until Caddy also
  forwards `/office/*` plus the unprefixed `/cache/`, `/coauthoring/` and
  `/doc/` paths OnlyOffice emits.
- `HW PL` and `MQL PC` hrefs carry a raw unencoded space. Browsers tolerate
  it; a stricter proxy may not. Untested through Caddy.

**Anchors**: `[D-129]` (rootless podman), FR-57 (`list_documents_url_prefix`).


## D-195: A source milestone's view shows where its documents land in the target

**Status**: Active — **Date**: 2026-09-08

**Context**: Every mapped DRR work-item is `no_customer_upload: true` with
`target_folder: null`, because DRR itself never submits — it is the
precursor milestone where the TPM checks for hurdles and waivers. So a DRR TG
page reported "not uploaded / work item is marked no_customer_upload" on
every row. True of DRR in isolation, and the opposite of what happens: those
documents DO reach the carrier under P1 via
`list_migrated_upload_files_for_item`. Eight identical cells saying "not
uploaded", on the one milestone whose entire purpose is feeding P1.

**Decision**: `resolve_carrier_destination` takes a `migrated_target_folder`
and, when an item is `no_customer_upload` but maps forward, returns the
TARGET item's path. A separate `migrated_to` field carries the human-readable
hop ("via DRR → P1 #14"), deliberately NOT folded into
`carrier_destination`: `UPLOAD-BUNDLE-1` uses that field verbatim as a zip
entry path and `UPLOAD-MANIFEST-1` as a collision key, so it must stay a
clean path.

The folder picker on a source milestone offers the TARGET milestone's
folders. DRR's own `target_folder`s are all NULL so its option list was
empty, and DRR documents are not listed under P1 at all — the view tree is
milestone-scoped and migration is a read-through at upload time — so the
source page is the ONLY place the override can be set. The override is
stored on the source association and travels through the migration.

**Consequences**:
- Exclusion precedence is unchanged and now pinned by tests: waiver, archive
  container, superseded and staged all still win over a mapped target, so a
  mapping cannot smuggle a waiver to the carrier.
- An unmapped `no_customer_upload` item still reports the exclusion — DRR #70
  and #76 are closed by the TPM directly and genuinely never deliver.
- A mapped target whose item has no `target_folder` keeps the exclusion
  rather than inventing a destination.

**Anchors**: `[D-153]` (one TG per document), `[D-191]` (upload-only
targets), `[D-193]` (fan-in), `UPLOAD-FLAT-1`, `UPLOAD-FOLDER-OVERRIDE-1`.


## D-196: Manual routing runs the FR-86 alignment check

**Status**: Active — **Date**: 2026-09-08

**Context**: `route_unrouted_to_item` wrote
`nsd_path_type=STAGED_NOT_CLASSIFIED` unconditionally. The
AUTO-CLASSIFY-RELNOTES-1 promotion sweep only fires when `doc_type` is
UNRESOLVED, so "already resolved and already aligned" had no path at all —
nothing to promote, and nothing to clear the staged state. A `test_report`
routed by hand onto MMK DRR #53 (`test_tech_waiver_report`, an ALIGNED pair)
sat staged forever: excluded from carrier upload AND from DRR→P1 migration,
under a reason string asserting a mismatch that did not exist. Its sibling,
routed automatically to the same item, classified fine.

**Decision**: three branches. UNRESOLVED promotes as before; resolved AND
FR-86-aligned resolves via the same `tpm_resolve_doc_type` call (which guards
on `nsd_path_type`, not on `doc_type` changing, so passing the doc_type the
file already carries still performs the `_staged_classification/ → rev1/`
move and the flip to CLASSIFIED); resolved and misaligned still stages, and
now logs both values.

DOCTYPE-WAIVER-VETO-1 is ported to this path. It shipped on the ingest path
only, so an UNRESOLVED `..._Waiver Request_*.ppt` manually routed onto a
compliance item was promoted by singleton alignment to
`compliance_certification_release_notes`, aligned, classified at rev1 and
uploaded — the exact incident the veto was written to stop. Only the
singleton rung needs guarding; `keyword_fallback_doc_type` tests for waiver
itself.

**Consequences**: per TPM 2026-09-08, routing is ONE-SHOT — `route_unrouted_to_item`
requires `routing_resolution=UNROUTED`, so a mistake is corrected by
Reclassify, not by re-routing. The reason string also stopped asserting an
unchecked mismatch: it now names both `doc_type` and `item_type`, and the
aligned case says what to do rather than something false.

**Anchors**: FR-86 (alignment invariant), FR-87 (TPM resolve steps),
`[D-190]` (doc_type precedence), `[D-192]` (unrouted dropdown scope).


## D-197: The manual-route dropdown annotates alignment rather than filtering on it

**Status**: Active — **Date**: 2026-09-09

**Context**: Routing and doc_type classification are independent — the router
never consults `doc_type`, and `doc_type` is finalised only AFTER routing
(promoted from the chosen item's `item_type` when the filename resolved
nothing). So a TPM can pick an item whose `item_type` does not align with the
document, and the file lands STAGED rather than delivered. The obvious fix is
to filter the dropdown to FR-86-aligned items only.

**Decision**: do NOT filter. Show `item_type` in the option label instead:
`#5 · MNO-UX · UX Compliance (Open · compliance_certification_release_notes)`.

**Why**: `[D-192]` already settled the same trade-off for TG scoping — an
empty dropdown leaves a TPM unable to route at all, which is worse than a
wide one. Filtering hits the same wall when no item in the TG aligns. And
when `doc_type` is resolved but WRONG, the honest fix is
Reclassify-then-route; filtering would block that path and leave nothing.
The Reclassify dropdown, by contrast, DOES filter to aligned options — it is
asserting "the slot is right, the label was wrong", which cannot strand
anyone.

**Consequences**: a mismatch stays reachable, but is now a deliberate choice
rather than an accident. Note the asymmetry this leaves: Reclassify always
resolves a conflict by changing the label to fit the slot, never by
questioning the slot — which is the direction that ships mislabelled waivers,
hence `[D-190]` and the veto in `[D-196]`.


## D-198: Process-local caches the dashboard reads must be loaded by build_app

**Status**: Active — **Date**: 2026-09-08

**Context**: `milestone_item_mapping`'s cache is process-local, and
`load_all_mappings()` had exactly one caller —
`workflow_engine.bootstrap.bootstrap_task_deps`, reached from celery_app's
init hook. That runs in hilda-worker and hilda-beat. hilda-api starts as
`uvicorn core.src.dashboard.app:build_app` and never runs it, so its cache
was empty and `get_target_item_no` returned None for every item.

The failure was silent by construction: a cache miss is indistinguishable
from a carrier with no mapping file, so every API caller took the "unmapped"
branch without an error, a warning, or a log line. `[D-195]` shipped and
changed nothing in the UI.

**Decision**: `build_app` loads the mapping itself, best-effort — a carrier
with no mapping file is the normal case, and a load failure must degrade the
destination column rather than take the dashboard down.

**Consequences**:
- The worse half was `build_milestone_manifest`, which also runs in
  hilda-api: the submission preview and download-all zip omitted EVERY
  migrated document, while `submit_to_carrier` — running in the worker,
  where the cache IS loaded — included them. A preview that disagrees with
  the submission is worse than no preview.
- Generalises: any process-local cache the dashboard reads has the same
  exposure. `template_lookup` is documented as "process-local and eager,
  loaded by bootstrap_task_deps" and should be audited the same way.
- `core/tests/test_dashboard_process_wiring.py` guards it and documents the
  trap. The API runs `--workers 2`, so the load happens per worker process.

**Anchors**: `[D-021]` (single hilda-api instance), `[D-193]`, `[D-195]`.


## D-199: Alembic revision ids stay ≤32 chars, and alembic.ini ships in the images

**Status**: Active — **Date**: 2026-09-07

**Context**: Applying migration `0005` on corp Postgres failed three times in
a row, each on a different cause. `alembic.ini`'s own header had documented
`alembic -c /app/alembic.ini current` since it was written, but the images
never COPYed the file. Copying it in by hand then failed with
`ModuleNotFoundError: No module named 'core'`, because `env.py` does `from
core.src.storage.db import Base` and the `alembic` console script puts its
own bin directory at `sys.path[0]`, not the working directory. Third:
`StringDataRightTruncationError` — the revision id
`0005_upload_target_folder_override` was the column's own name at 34
characters, and alembic creates `alembic_version.version_num` as VARCHAR(32).
Because the DDL and the version bookkeeping share one transaction, the ALTER
rolled back too, so the column never landed and `current` still read 0004.

**Decision**: revision ids are capped at 32 characters (renamed to
`0005_upload_folder_override`); `prepend_sys_path = .` in `alembic.ini`,
resolved against the cwd — `/app` in the containers, the repo root in local
dev; and `COPY alembic.ini /app/alembic.ini` in Dockerfile.hilda-worker and
Dockerfile.hilda-api. Not hilda-beat: the scheduler is not a migration entry
point.

**Why cap rather than widen the column**: a fresh database recreates
`alembic_version` at VARCHAR(32), so a widened column is silently undone the
next time the schema is built from scratch — which was about to happen on the
corp box.

**Consequences**: nothing in the suite would have caught any of this — tests
build schema from `Base.metadata.create_all()`, so no test runs a migration.
`core/tests/test_migration_revision_ids.py` closes that gap: id lengths, one
base and one head with every `down_revision` resolving, and filenames
matching revision ids. `0003` is already 29 characters, so the margin is thin
