...
## D-173: NSD folder-name tag routing via `match_hint` (two-input router separation)

**Status**: Active · **Date**: 2026-08-24

**Context**: NSD2 ingest walks Samsung SMB shares whose folder layout carries tag semantics that HILDA's Fr52 attachment router needs (e.g. `HAC T-Coil/report.pdf` — the folder name "HAC T-Coil" is the routing signal; the leaf filename "report.pdf" is generic). But the same attachment goes through TWO independent routing checks: (1) `_tg_scoped_route` substring-matches against each candidate item's `item_description` tag-sets to pick WHICH work item (item_description contains fragments like "HAC", "T-Coil"); (2) `_classify_doc_type` regex-matches against a filename-pattern registry to pick WHICH doc_type ("test_report" / "waiver" / etc). The signals want DIFFERENT inputs — folder-path drives (1), leaf filename drives (2). Two-input separation was needed without breaking email ingest (attachments arrive with filename only, no folder context) or PLM ingest (filenames sometimes carry tag hints inside them). Options considered:
- **Option A (concatenate folder + filename)**: `filename = "HAC T-Coil/report.pdf"` for BOTH stages. Doc_type regex would need `.` -not-in-filename gate, break PDFs that arrive without folder context, and pollute doc_type classification with folder noise.
- **Option B (separate `match_hint` field on `InboundAttachment`)**: Add `match_hint: str | None = None` alongside existing `filename`. Router uses `match_hint or filename` for tag matching; keeps `filename` for doc-type classification. NSD ingest sets `match_hint = <parent_folder>`; email/PLM ingest omits it. Zero impact on non-NSD paths.

**Decision**: Option B — `InboundAttachment` grows `match_hint: str | None = None` field. Router's `_route_to_items` splits the two signals internally: `filename = (attachment.filename or "").lower()` for doc-type classification; `match_input = (attachment.match_hint or attachment.filename or "").lower()` for `_tg_scoped_route` item_description matching. NSD2 poll sets `match_hint = PurePosixPath(filename).parent.name or None` on every ingested attachment. Multi-item shared-tag folders (e.g. a "HAC" folder that could match 3 items whose item_description all contain `[["HAC"]]`) resolve via the existing `TG_DEFAULT_MULTIMATCH` mechanism from `[D-159]`: TPM marks one item's tag-set with `["default"]` — that item wins the tie.

**Why**:
- **Two inputs, two signals**. Folder-path and filename are semantically different in NSD ingest — collapsing them into one string forces every downstream regex + substring rule to accommodate both shapes, growing the surface where bugs hide.
- **Backwards-compatible default**. `match_hint: str | None = None` — every existing caller (email path, PLM path pre-PLMARCH, tests) works unchanged. Only NSD poll sets it. No dual-write, no migration.
- **Router internals stay explicit**. Two named locals (`filename` for classification, `match_input` for tag matching) make the intent readable at the call site. Contrast a hidden "use folder if present" heuristic buried in `_tg_scoped_route`.
- **Reuses TG_DEFAULT_MULTIMATCH for shared folders**. TPMs already understand the `["default"]` tag convention from `[D-159]`. Shared-tag folders don't need a new resolution mechanism — same tiebreaker, same UI, same test coverage.
- **Reuses storage-side dedup**. `document_item_association`'s composite key `(document_id, delivery_item_id)` already allows one file_hash → multiple item associations. NSD folder shared by 3 items produces 1 `document_index` row + N `document_item_association` rows if needed (though `["default"]` tie-break collapses to 1 assoc in practice).
- **Not concatenation**: matching on "HAC T-Coil/report.pdf" would false-positive item_descriptions containing "report" as a tag fragment; keeping the two signals separate avoids that.

**Consequences**:
- **`InboundAttachment` is now a two-input contract**. Any future ingest path (SFTP walker, WebDAV crawler, Teams channel scraper) that has richer context than a bare filename should populate `match_hint` with the routing signal.
- **Router regression surface widens by one field**. `_route_to_items` tests must cover: match_hint=None (email path — backwards compat), match_hint present but same as filename (edge case; matches like today), match_hint different from filename (folder-drives-routing case), match_hint empty string (defensive: falls back to filename per `or` chain).
- **NSD walk depth matters**. `PurePosixPath(filename).parent.name` returns the IMMEDIATE parent folder. Deep nested folders (`carrier/device/milestone/HAC/T-Coil/report.pdf`) only surface "T-Coil" as match_hint — sufficient for current NSD2 layout per `[D-168]` but locks the tag-in-parent-folder-only shape.
- **No template.yaml change required**. Tag-sets in `item_description` continue to reference folder-fragment substrings ("HAC", "T-Coil") — same authoring model TPMs use today. Users don't need to change how they set up template.yaml when NSD ingest joins.
- **NSD `.7z` inside a tagged folder**: PLMARCH cascade (`[D-175]`) threads `ingest_source` through recursive archive dispatch; NSDMATCH sets `match_hint` on the outer archive attachment only. Inner extracted attachments get filename-only routing per the archive extractor's contract — folder context is lost past the archive boundary. Acceptable per current NSD file layout (tagged folders contain leaf files, not zips of leaf files).

**Anchors**: `[D-159]` (TG_DEFAULT_MULTIMATCH `["default"]` tiebreaker — reused); `[D-168]` (NSD2 walk shape — establishes the folder layout this routing depends on); `[D-170]` (tracking_modality opt-in — NSD2 poll only runs where SP marks it); `[D-155]` (archive container semantics — inner-file routing loses folder context past archive boundary); commit `0e2013c` (NSDMATCH-1..4 cascade).

## D-174: TPM UI reclassification for Unresolved doc_type (FR-87 step B closes; ingest-source-agnostic)

**Status**: Active · **Date**: 2026-08-24

**Context**: When Fr52 attachment router lands a file but the doc_type regex classifier finds no match (unknown format, ambiguous keywords, or new template not yet added to the classifier registry), the file gets `doc_type="unresolved"` + `nsd_path_type=STAGED_NOT_CLASSIFIED` — bytes land in `internal/_staged_classification/` view-tree path, awaiting manual triage. FR-87 originally scoped a TPM UI to reclassify these ("step B — reclassify doc_type"); step A (route unrouted TG assignment) shipped as UR-5/6/7 for the `/_unknownTG/` bucket, but step B was deferred as a Ph-2 flag while ingest cascades caught up. Recent NSD2 + PLM ingest paths increased Unresolved volume enough that manual triage via direct SQL update became a bottleneck (TPMs asking for the UI). Design options:
- **Option A (ingest-time slug regeneration at reclassify)**: keep doc_id_slug empty at ingest for Unresolved; when TPM picks a doc_type in the UI, derive slug via `_slug_from_filename` + rev=1. Preserves NEW_DOCUMENT semantics — the file is being classified for the first time, so it enters revision 1 fresh.
- **Option B (slug-at-ingest, rev-bump-at-reclassify)**: assign doc_id_slug at ingest even for Unresolved, then treat reclassify as a revision update. Complicates the ingest path (need to peek at filename shape when doc_type is unknown), and semantically wrong — the doc has never been classified, so calling reclassification "rev 2" misrepresents history.

**Decision**: Option A. UI wired at `POST /browse/{customer}/{device}/{milestone}/tg/{tg_name}/reclassify` with form fields `file_hash` + `new_doc_type` (dropdown of 4 concrete DocType enum values: `test_report` / `tech_report` / `waiver` / `compliance_certification_release_notes`). Handler derives slug via `Fr52AttachmentRouter._slug_from_filename` + rev=1 (matching ingest-time slug generation for NEW_DOCUMENT), fetches TPM email from `Projects_<customer>` SP list via `_read_tpm_email` (falls back to principal / `tpm@unknown` sentinel per SETUP-1 tolerance), then delegates to existing `tpm_resolve_doc_type` storage helper (unchanged public surface — was already the write-path for FR-87 step B's future UI). Post-Redirect-Get with 5 flash-banner outcomes (`reclassified` / `not_staged` / `invalid_doc_type` / `no_doc_row` / `partial_failure`). TG-view template gets a new "Doc Type" column + inline Reclassify form (dropdown + confirm modal) on rows where `is_staged=True`. Ingest-source agnostic: same UI works for Email / PLM / NSD-ingested Unresolved docs — the reclassify path doesn't care where the bytes came from, it operates on `document_index.file_hash` regardless.

**Why**:
- **Closes long-standing Ph-2 flag** (FR-87 step B) without inflating scope. Only the missing UI piece is added; the storage helper (`tpm_resolve_doc_type`), the doc_type enum, the STAGED_NOT_CLASSIFIED path convention, and the CLASSIFIED promotion mechanism were all already in place from earlier ingest work.
- **Slug derived on-demand at reclassify, not at ingest**. Preserves the "first classification = rev 1" semantic. If HILDA ever expands the doc_type classifier registry and re-ingests old Unresolved files against the widened rules, those files would land as classified NEW_DOCUMENT with rev=1 — consistent with the manual UI path.
- **Ingest-source agnostic UI**. NSD, PLM, Email Unresolved docs all land in the same STAGED_NOT_CLASSIFIED table with the same file_hash key. One UI + one route + one handler covers all three ingest sources. Contrast the alternative of source-specific reclassify UIs (would triple the surface for zero gain).
- **Reuses `tpm_resolve_doc_type`**. The storage helper handles: file move from staging path to classified path, `document_index.doc_type` upgrade, `document_item_association.nsd_path_type` promotion (STAGED_NOT_CLASSIFIED → CLASSIFIED), `local_nsd_path` rewrite. Zero new storage logic; UI just calls the helper.
- **Post-Redirect-Get with flash banners**. Standard web form UX; TPMs see immediate feedback in the flash area of the redirect target, browser back button doesn't re-POST.
- **`is_staged=True` gate on the UI**. Reclassify form only renders on Unresolved rows — already-classified docs don't offer the dropdown. Prevents accidental doc_type churn on stable files.
- **TPM email fallback tolerance** (SETUP-1 pattern). Real corp SP sometimes returns nulls or missing People fields on Projects list rows; fallback to principal + `tpm@unknown` sentinel keeps the UI functional in that case rather than hard-failing the reclassify.
- **View path unchanged across reclassification**. View-tree layout `_data/view/<customer>/<device>/<milestone>/<tg>/<filename>` doesn't include doc_type as a segment — reclassifying doesn't move the file within the view tree, so TPM's browse position stays put. Only the internal classified-storage path moves.

**Consequences**:
- **RECLASS-BUGFIX regression** (commit `d00c3ec`, same-day fix): `tpm_resolve_doc_type` was calling `NSDPath.internal_classified(customer, device, milestone, tg, item, new_doc_type, doc_id_slug, rev_number)` — 8 positional args, missing the 9th `original_filename` (defaults to `""`). `internal_classified` docstring at `nsd.py:73-83` explicitly warns: with empty original_filename, the factory returns a DIRECTORY-only path (`.../slug/revN`). `_move_nsd_file` then wrote bytes to that path as a FILE literally named `revN` at the doc_id_slug level, not a file INSIDE `revN/`. Fix: thread `original_filename=original` as the 9th arg (var already computed on `document_ops.py:702` from `DocumentIndexTable.original_filename`). Regression guard: happy-path test now asserts `.../rev1/<filename>` tail exactly. Same defensive pattern already applied to the ingest-time call site 2026-06-30; the reclassify call was overlooked because it landed after and wasn't in the diff at that time.
- **Doc_type dropdown is closed set** (4 concrete DocType values, excludes ARCHIVE_CONTAINER + unresolved). Adding new doc_type classifications requires enum extension + dropdown update. Explicit rather than free-text — TPMs can't invent doc_types.
- **RECLASS-4 test suite**: 3 TestClient e2e tests exercising the full stack — form parse → validation → Projects SP lookup fallback → slug derivation → tpm_resolve_doc_type call → real filesystem move. Happy-path asserts CLASSIFIED promotion + local_nsd_path contains new doc_type segment + `.../rev1/<filename>` tail.
- **TgFileEntry schema growth**: `TgFileEntry` gains `doc_type: str = ""`, `file_hash: str = ""`, `is_staged: bool = False`. `list_files_in_tg` extends with a batch join to `document_index + document_item_association` on file_hash so UI can render the columns + gate the form. Both underlying tables already indexed on file_hash; join is cheap.
- **Precedent for other Unresolved-triage UIs**. When a future field needs TPM in-UI reclassification (e.g. per-item TG re-assignment when router picks wrong), this same pattern applies: extend `list_*` storage helper with the join, add UI columns + inline form, POST handler delegates to existing storage write-path, PRG + flash banners.
- **No customization.yaml impact**. Purely an ingest-side / TPM-facing UI change; no SP schema shift, no template.yaml addition.

**Anchors**: `[D-158]` (FR-87 step A — `/_unknownTG/` manual routing UI, sibling pattern); `[D-152]` (STAGED_NOT_CLASSIFIED / CLASSIFIED nsd_path_type semantics — established the state machine this UI drives); `[D-160]` (DocType enum ARCHIVE_CONTAINER sentinel — closed-set enum shape reclassify dropdown respects); SETUP-1 (`_read_tpm_email` fallback tolerance — reused); FR-87 (requirements anchor); commits `2b0730d` (RECLASS-1 TgFileEntry extension), `ac10d0f` (RECLASS-2/3 route + template), `264ca8e` (RECLASS-4 tests), `d00c3ec` (RECLASS-BUGFIX original_filename thread), RECLASS-BUGFIX-2 (2026-08-26, addendum below).

**Addendum 2026-08-26 (RECLASS-BUGFIX-2)**: Live corp box UI showed `compliance_certification_release_notes — not classified` on an already-classified row, with the Reclassify dropdown spuriously offered. Root cause: `is_staged` computation joined `document_item_association.nsd_path_type=STAGED_NOT_CLASSIFIED` — a filesystem-location signal that got out of sync with `document_index.doc_type` (SQL confirmed: doc_type was `compliance_certification_release_notes` while assoc's nsd_path_type stayed at STAGED). Fix: `is_staged` is now doc_type-driven — `doc_type_by_hash.get(sha256, '__missing__') in ('', 'unresolved')`. Sentinel distinguishes "no `document_index` row" (legacy/vintage doc — don't offer Reclassify) from "index row with unresolved doc_type" (offer it). The STAGED_NOT_CLASSIFIED assoc query is dropped entirely. Template caption changed from `{{ doc_type or 'Unresolved' }} — not classified` (contradictory when doc_type is real) to `Unresolved — pick a type to classify` (safe under all conditions; defensive text since the fixed predicate ensures it only renders when doc_type IS unresolved). Refined design point: **doc_type on document_index is source of truth for classification state; nsd_path_type on document_item_association is source of truth for filesystem location — they should agree post-CLASSIFIED promotion but if they diverge, doc_type wins for UI-gating purposes**. The underlying inconsistency (some assocs stuck at STAGED_NOT_CLASSIFIED after doc_type resolution — probable NSDMATCH/PLMARCH archive-inner ingest edge case) is tracked as a separate STATUS Flag investigation (may fold into a proposed reconcile sync-7 alongside TEMPLATE-BACKFILL-1). 1 new regression test (`test_classified_doc_type_wins_over_stale_staged_assoc`) locks the predicate against re-regression.

## D-175: Archive extraction parity across all ingest sources (`ingest_source` param threaded through recursive dispatch)

**Status**: Active · **Date**: 2026-08-24

**Context**: `[D-155]` established archive-as-container semantics for `.zip` + `.7z`: outer archive gets no router match / no doc_type classification (audit-only `document_index` row with `doc_type=ARCHIVE_CONTAINER`), inner entries extracted and routed independently through the per-attachment pipeline. `[D-160]` (NEST-1) added recursive archive extraction so zip-in-zip / 7z-in-zip cases work with depth-cap + cycle detection + size caps. But the dispatch was email-only: `_process_archive_attachment` was called from `email_polling` → `inbound_attachment.py`, threading no `ingest_source` value (defaulted internally to `IngestSource.EMAIL.value` at line 473). When NSD2 + PLM ingest paths shipped (`[D-168]` / `[D-169]`), their attachment loops used a simpler flow that treated the outer archive as an opaque blob — one `document_association` on the outer file, inner files never surfaced. Live corp box symptom: TPM received PLM ticket with a zip of test reports → HILDA created one association tagged `CorporatePLM` for the outer `.zip` → submit-to-carrier upload found only the zip container, not the individual PDFs → carrier upload effectively empty. Same shape for NSD2 walks that hit a folder containing `.7z`. Options:
- **Option A (per-source archive dispatch, three copies of the logic)**: each of `email_polling`, `plm_poll`, `nsd2_poll` gets its own `_process_archive_attachment`-equivalent. Fastest to implement but triples the divergence surface — bug fixes need three edits, size caps + depth caps + cycle detection duplicated three times.
- **Option B (thread `ingest_source` through the existing recursive dispatch)**: `_process_archive_attachment` grows `ingest_source: str | None = None`, param threaded through `_process_regular_attachment` inner call + recursive self-call. PLM + NSD ingest paths gain a `_is_archive_attachment(attachment)` dispatch guard that routes through the shared function when true. Email path unchanged (defaults still produce `EMAIL` ingest_source).

**Decision**: Option B. `_process_archive_attachment(deps, router, attachment, candidate_items, batch_id, correlation_id, ingest_source: str | None = None)` — new keyword-only param with `None` default preserves email-path behavior. Inside the function: recursive self-call for zip-in-zip cases passes `ingest_source=ingest_source` unchanged; `_process_regular_attachment` inner call takes the same value; the outer `document_index` ARCHIVE_CONTAINER audit row uses `ingest_source or IngestSource.EMAIL.value` so email path stays a bare `EMAIL` string, PLM path writes `CorporatePLM`, NSD path writes `NetworkSharedDrive`. Both `plm_poll._ingest_new_plm_file` and `nsd2_poll._ingest_new_nsd2_file` gain a `_is_archive_attachment(attachment)` dispatch check up-top: on true, delegate to `_process_archive_attachment` with the caller's IngestSource; on false, fall through to the existing regular-attachment path.

**Why**:
- **One archive-handling codebase, three call sites**. Depth-cap, cycle-detect (file_hash tracking), decompressed-size cap, password-protected fail-open, bad-magic fail-open, view-tree replication semantics — all live in one function. NSD/PLM ingest inherit all of them for free by delegating.
- **`ingest_source` as opt-in param preserves backward compat**. Every existing caller (email path, tests, PLMARCH-4 backward-compat coverage) works unchanged. Only the new PLM + NSD sites pass the value.
- **Inner-file ingest_source matches outer**. If a PLM zip contains a PDF, that PDF's `document_index.ingest_source = CorporatePLM` — same as if TPM had uploaded the PDF standalone to the same PLM ticket. Consistent semantics; downstream analytics/reports don't see phantom `EMAIL` rows.
- **`_is_archive_attachment` dispatch is one predicate**. Extension-based (`.zip` / `.7z`) — same predicate email path uses. Consistent classification across ingest sources.
- **Avoids per-source divergence during bug fixes**. When `[D-155]` outer-archive replication was later refined (added view-tree replication so TPMs see the archive in the matched-TG view alongside the extracted files) — that change lands in one place and propagates to all three ingest paths automatically. Contrast Option A where each fix needs three edits + drift risk.

**Consequences**:
- **`_process_archive_attachment` signature stability matters more now**. Three callers → API contract; any future param change is a breaking change across three ingest paths. Tests should cover all three sources.
- **PLMARCH-4 tests establish coverage baseline**: (1) PLM archive tags outer + inner as `CorporatePLM` (asserts `document_index.ingest_source` on both rows), (2) NSD archive tags both as `NetworkSharedDrive`, (3) email archive preserves the `EMAIL` default when `ingest_source=None` — regression guard for backward compat. Every future ingest source added should add its own PLMARCH-style test.
- **ARCHIVE_CONTAINER audit row propagates ingest_source**. `[D-160]` DocType sentinel — the outer-archive audit row is still opaque routing-wise (`routing_resolution=ArchiveContainer`, no `document_item_association`), but now its `ingest_source` is truthful. UR-3 unrouted lister still excludes ARCHIVE_CONTAINER doc_type per ARCH-UNROUTED-1 (`[D-155]` design-collision fix same session).
- **View-tree replication (D-155 addition) applies to all sources**. The outer archive's bytes get written to `_data/view/.../` in every matched TG so TPMs see "here's the archive these files came from" — now this behavior applies to PLM + NSD archives too, not just email. Ops-visibility win.
- **NSD folder-tag routing (D-173) doesn't cross the archive boundary**. `match_hint` set on the outer archive attachment (parent folder name) doesn't propagate to inner extracted attachments — the archive extractor produces plain filenames without folder context. Documented limitation; acceptable per current NSD layout where tagged folders contain leaf files, not zips.
- **PLM ticket with mixed archive + loose files**: PLM poll's download dir walk fans out N attachments — each goes through `_is_archive_attachment` dispatch. Mixed batches (one .zip + two PDFs) route correctly per-file.
- **NSD folder walk finds `.7z`**: same dispatch — outer archive audit row + inner file routing per candidate items in scope.

**Anchors**: `[D-155]` (archive-as-container semantics — established the `_process_archive_attachment` shape); `[D-160]` (DocType.ARCHIVE_CONTAINER sentinel + NEST-1 recursion + cycle-detect — the machinery this decision extends); `[D-168]` (NSD2 poll — one of the newly-covered sources); `[D-169]` (PLM poll — the other newly-covered source); `[D-173]` (NSDMATCH `match_hint` — companion two-input change, boundary-limited by this decision); ARCH-UNROUTED-1 (STATUS Done — related same-session cleanup of UR-3 lister to exclude ARCHIVE_CONTAINER audit rows); commit `f4ae36b` (PLMARCH-1..5 cascade).

## D-176: reconcile sync-7 retry-unrouted — closes the ingest-tick / item-import timing race

**Status**: Active · **Date**: 2026-08-26

**Context**: NSD2 + PLM polling ticks fetch their eligible-item candidate pool ONCE per tick via `list_items_for_milestone(...)` + `_filter_hw_pl_nsd2_items(...)`. When a file arrives whose `match_hint` (folder name, per `[D-173]` NSDMATCH) would substring-match a delivery_item's `item_description` tag — but that delivery_item was created in Postgres AFTER the tick's candidate-pool snapshot — the router sees an empty candidate set for that hint → file lands in `_unrouted` via document_index audit row + `internal_default_workitem` view path. The next tick's file_hash dedup skips the file (already in document_index), so no automatic recovery. Live corp-box surface (2026-08-26 CEC investigation): 30+ CEC/HAC/FCC files stranded because items 21/22/23/108-111 were imported via SP alert AFTER prior NSD ticks captured only 6 candidates {7, 8, 9, 15, 19, 20}. Only recovery paths were: (a) manual `/browse/.../_unknownTG/` UI triage per file (UR-5/6 shipped 2026-07-28), or (b) bulk DELETE of document_index rows to force re-ingest. Neither scales. Options considered for automated recovery:
- **Option A (retry-at-ingest-time)**: change NSD/PLM tick to re-fetch candidates immediately before each file's routing decision (per-file, not per-tick). Closes the tick-scoped race, but doesn't help files already ingested BEFORE the item existed — the file has to be RE-ingested for the fresh candidate pool to fire, and dedup blocks that. Doesn't solve the actual problem.
- **Option B (retry-at-reconcile-time)**: new sync in the 5-sync meta-reconciler (`[D-142]`) that periodically inspects `_unrouted` docs and re-runs the router substring-match against the CURRENT candidate pool. When a match now exists, promote the association via existing `route_unrouted_to_item`. Idempotent; converges naturally; leverages the reconcile-cascade infrastructure already in place.
- **Option C (event-driven recovery)**: hook a "new-item-created" signal from `sp_alert_imports` → trigger a scoped retry-unrouted pass. Cleaner event model, but adds a new event type + subscriber wiring + coupling between two subsystems that today have no direct dependency. Postponed.

**Decision**: Option B — reconcile sync-7 (retry-unrouted). Added to `reconcile_config.ReconcileConfig` as `sync_7_retry_unrouted: SyncTypeConfig(enabled=True, elapsed_threshold_sec=0)` — fires immediately per meta-tick since the predicate (unrouted + matching item now exists) has no timing race with any concurrent path. Wired into `reconcile_all_task`'s per-tuple loop after sync-6. Implementation `_sync_7_retry_unrouted`:
1. `list_unrouted_for_scope(customer, device, milestone)` — every DocumentIndex row in scope with no association.
2. `list_route_candidates_for_scope(...)` — current eligible-target pool (Confirmation + Default excluded, per manual-triage semantics).
3. For each unrouted doc:
   - Derive `match_hint = PurePosixPath(original_filename).parent.name.lower()` (NSDMATCH-3 pattern from `[D-173]`).
   - Empty parent → silently skip (email-ingested or root-level; no folder context to rescue).
   - Substring-match against each candidate's `item_description` via `_matches_any_tag_group` (local mirror of Fr52 AND-of-OR semantics; IMEI reserved-literal branch omitted since folder-name path doesn't apply).
   - **Exactly 1 match** → `route_unrouted_to_item(file_hash, target, tpm_id='reconcile-sync-7')` + `_audit(reconcile_sync_7_retry_unrouted_routed, ...)`.
   - **0 matches** → `stats["sync_7_no_match"] += 1` (candidate pool still doesn't have a matching item; leave for TPM triage or future item add).
   - **≥2 matches** → `stats["sync_7_multi_match"] += 1` (ambiguous; leave for TPM triage; conservative default respects `[D-153]` cross-TG constraint intent).

**Why**:
- **Fixes the timing race without changing ingest-tick shape**. NSD/PLM tick logic stays unchanged (per-tick candidate pool remains the norm — cheaper than per-file DB fetches). Recovery happens out-of-band on a different cadence (default `interval_sec=300`).
- **Idempotent + naturally converging**. Once a file is routed, `list_unrouted_for_scope` no longer surfaces it (association exists). Sync-7 does its work once per stuck file, then stops. No retry-limit tracking needed.
- **Zero-timing-threshold is correct here**. Unlike sync-1..5 which back off `elapsed_threshold_sec=300..900` to avoid racing the still-in-flight alert path, sync-7's predicate is unambiguous: "unrouted AND a matching item exists" — no in-flight path could produce this condition; fire immediately.
- **Conservative multi-match handling**. Only the unambiguous single-TG single-item case is auto-promoted. Cross-TG ambiguity + intra-TG ambiguity (multiple items in same TG matching same tag) defer to TPM manual triage via existing UR-5/6 UI. Preserves `[D-153]` intent + prevents auto-promotion errors that would need a rollback UI.
- **Existing infrastructure**. Reuses `list_unrouted_for_scope` (UR-3), `list_route_candidates_for_scope` (UR-3), `route_unrouted_to_item` (UR-6 storage helper), `_audit` (reconcile pattern), `Fr52AttachmentRouter._extract_tag_groups` (static helper). Only new code: sync-7 function + 4-line predicate `_matches_any_tag_group`.
- **Substring predicate is local**, not a call into the router class. Avoids pulling `Fr52AttachmentRouter` instance construction into reconcile.py (which would drag in router config + candidate widening). Sync-7 doesn't need doc-type classification, view-tree write, or archive dispatch — just the "which item's tag matches this folder" question.
- **Email/PLM ingest safety**: empty-parent-folder skip means email attachments (typically root-level filenames) don't get retried via folder-based logic. PLM downloads landed in `_unrouted` similarly skipped unless the PLM ticket happened to place files inside a folder path (rare).

**Consequences**:
- **New sync stat keys**: `sync_7_routed`, `sync_7_multi_match`, `sync_7_no_match`, `sync_7_skipped`. Present in every `reconcile_all_task` return dict; ops digest telemetry gets 4 more counters to trend.
- **Meta-tick cost increase**: each tuple now runs 7 syncs (was 6). Sync-7 adds 1 storage read (`list_unrouted_for_scope`) + 1 storage read (`list_route_candidates_for_scope`) + N per-doc `route_unrouted_to_item` calls when matches fire. Storage reads are indexed on (customer_id, device_id, milestone_id) per UR-1 columns; typically <100 unrouted docs per scope; negligible.
- **`tpm_id='reconcile-sync-7'` sentinel** in `communication_log.actor_id`. Distinguishes auto-promoted files from TPM-manual routes in audit trail. Grep-friendly.
- **Precedent for other reconcile-recovery patterns**. When a future ingest race is discovered (say, PLM ticket close before its ticket-poll returns → some doc stranded), same pattern applies: add sync-N with a stateless predicate over Postgres; no timing threshold; delegate to existing storage helpers; conservative auto-promotion. TEMPLATE-BACKFILL-1 Ph-2 candidate (from STATUS Flags) will likely follow this shape.
- **Doesn't touch dedup logic**. Deliberately: dedup is correct at ingest time; changing it would risk double-ingestion for legitimate re-processing scenarios. Sync-7 works AROUND dedup by acting on already-ingested rows.
- **12 new tests** (`TestMatchesAnyTagGroup` × 6, `TestSync7RetryUnrouted` × 6): predicate semantics + full sync-7 pass covering disabled-config skip, single-match promote + audit, multi-match skip, zero-match skip, no-folder-context silent skip, and empty-unrouted early return. 51/51 test_reconcile pass.
- **Ops migration**: no template.yaml change, no SP schema change, no Postgres migration. `podman restart hilda-beat hilda-worker` after bind-mount refresh + next `reconcile_all_task` tick sweeps stuck files. Corp-box CEC / HAC / FCC stranded files auto-route on first sync-7 tick.
- **Kept sync-7 in reconcile.py rather than a new task file**. The 5-sync meta-reconciler pattern is intentionally batched — one beat entry orchestrates them all serially per tuple. Adding a separate `retry_unrouted_task` would double-fetch the tuple iteration + duplicate the meta task boilerplate for no gain.
