...
- **FR-39** — PM team leads author customer templates via the SharePoint UI and / or Microsoft Excel upload; uploaded Excel templates conform to the per-customer schema generated by the Template Schema Ingestor `[D-010]`.
...
- **DEF-5** — Messenger adapter full bidirectional (§7.2) (deferred: POC only in v1; full feature v2 — revisit: when v1 messenger choice is resolved per `STATUS.md` Flags).
- **DEF-6** — Proprietary internal IssueTracker adapter generated by the API Spec Ingestor end-to-end (`[D-003]`) (deferred: v1 IssueTracker target is Jira; proprietary adapter is the v1-acceptance milestone if `[D-009]` v1 messenger choice picks the proprietary path — revisit: same trigger as DEF-5).
