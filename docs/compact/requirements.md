...
- **FR-55** `[Ph-1]` — For DeliveryItems whose `tracking_modality includes NetworkSharedDrive`, HILDA monitors the item's `inbound/` drop folder on the shared network drive by configurable-interval polling; when a new file is detected (written directly by an owner with filesystem write access), the item is identified from the folder path (no FR-52 routing needed), the file is recorded in `CommunicationLog` with the owner's filesystem identity, the DeliveryItem record is updated with the attachment link, document arrival triggers a `delivery_state` transition (configurable rule), and the PLM auto-upload per FR-13 is triggered; detection latency is bounded by the configurable poll interval (default ≤ 5 minutes); HILDA does **not** poll drop folders for items where `tracking_modality` does not include `NetworkSharedDrive`.
- **FR-52** `[Ph-1]` — When an inbound email contains attachments, the system routes each attachment to the corresponding DeliveryItem via a two-tier process (anchors `[D-033]`): (1) extract first-page text from each attachment and fuzzy-match against item name, description, and `item_type` within the batch — high-confidence fuzzy matches are applied directly; (2) for attachments where fuzzy matching cannot resolve the mapping, the runtime LLM inspects the first-page content together with the batch DeliveryItem list to determine the match; when the triggering email also fires FR-12 path (c), attachment routing is subsumed into the single fused LLM call for that path and the two-tier process does not run separately (anchors `[D-034]`); confirmed matches are written to the DeliveryItem's `inbound/` subdirectory on the shared network drive per FR-13 and the DeliveryItem record is updated with the attachment link; unresolved attachments are surfaced on the PM dashboard for manual assignment.
...
- **FR-26** `[Ph-1]` — Corp PLM issues are created once per (owner × milestone) at tracker creation (FR-2), not per DeliveryItem; the PLM issue is **document-only** — corp PLM does **not** sync status back to HILDA; when `tracking_modality includes CorporatePLM`, HILDA polls the PLM issue at a configurable interval for new owner-uploaded documents and copies each to the shared drive (FR-13); when `tracking_modality does not include CorporatePLM`, HILDA auto-pushes every new `inbound/` document to the PLM issue instead (FR-13); in all cases corp PLM is the document source of truth (anchors `[D-035]`); there is no internal-Jira tracking — the Jira adapter is used only for customer-facing `CustomerJIRA` modality per FR-25.
...
- **FR-54** `[Ph-1]` — Inbound corp messenger messages from owners are captured on receipt, attributed by sender handle to the owner, recorded as comments on all open DeliveryItems in the owner's active batch, and surfaced as a `Messenger reply — Manual triage` flag on the PM dashboard; if the owner has multiple open items across milestones, flags are grouped by milestone to minimise PM triage actions; `[Ph-2]` the runtime LLM classifies the message content to extract status (closed / open / delayed / blocked) and reason — above-threshold classifications are auto-applied; below-threshold go to PM triage.
