...
- **FR-70** `[Ph-1]` — Each DeliveryItem has an `item_completion_pct` computed field (0–100 integer) representing document-review progress across all received revisions; computed as: (count of document index rows for this item with non-null `llm_review_findings`) / (total document index rows for this item) × 100; `Confirmation` items always have `item_completion_pct = 100` (no documents expected); `item_completion_pct` is recomputed on every new document index write (new revision, new `llm_review_findings` write, or new `parser_result` write); the field is persisted on the `DeliveryItem` SharePoint List row and is available in the milestone view (FR-56) for PM monitoring; Milestone-level `milestone_completion_pct` (FR-6) is the average of its child DeliveryItems' `item_completion_pct` values.
