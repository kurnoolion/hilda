...
- Fill in `docs/compact/PROJECT.md` during requirements phase (TODO placeholders + Open questions).
- Populate `docs/compact/requirements.md` with v1 FR set and NFRs covering data sensitivity, audit, PM-approval gate, credential isolation, on-prem boundary, SharePoint-2017 constraint, near-real-time tracking SLO.
- Open questions in `PROJECT.md`: 4 resolved during this session (SharePoint API → D-006; LLM hosting → D-007; intermediate-Protocol design → D-008+D-009; customer-template authoring partial → D-010); 5 backlogged below in Flags. No active blocking questions.
