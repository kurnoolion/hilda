...
**Milestone-scoped writes go to the milestone sentinel row.** Milestone-level actions (Start Collection / Submit to Carrier / Close All Items / Refresh / Download Package) write to a **sentinel row** on the Milestones list representing milestone-level state. Per-DeliveryItem actions write to the individual DI row.
...
## Milestone View — top-bar buttons (sentinel-row writes)
...
| **What happens on click** | Set `Milestones.milestone_collection_started_at = <now>` on the milestone sentinel row. |
| **What HILDA does next** | *(Background — out of your scope)* HILDA transitions each DI from `Not Started` → `Open`, creates PLM issues per (owner × milestone), fires email outreach to all owners, transitions each DI to `OutreachSent`, activates runtime polling channels (Email/NSD/PLM/CustomerJIRA per per-item `tracking_modality`), and sets `Milestone.status = "In Progress"`. Item rows update via live polling. |
...
| **What happens on click** | Set `Milestones.milestone_submission_triggered_at = <now>` on the milestone sentinel row. |
...
| **What happens on click** | Set `Milestones.close_all_items_triggered_at = <now>` on the milestone sentinel row. |
...
| **What happens on click** | Set `Milestones.refresh_requested_at = <now>` on the milestone sentinel row. |
...
| **What happens on click** | (a) Set `Milestones.download_package_request_timestamp = <now>` on the milestone sentinel row; (b) immediately update SP UI to show "Package preparing..." indicator (replaces the button); (c) clear any prior `download_package_url`, `download_package_status`, `download_package_generated_at` on the same row (fresh assembly per click). |
| **What HILDA does next** | *(Background — out of your scope)* HILDA assembles the zip in background per FR-73 (individual files only per FR-77 Type-2 routing — no audit zips). On completion, HILDA writes back: `download_package_url = https://hilda-proxy.corp/dl/<scoped_token>`, `download_package_status = "ready"`, `download_package_generated_at = <now>`. On failure: `download_package_status = "failed"` + `HildaOpsAlert` logged. |
| **Live polling on completion** | SP web part's focus-aware refresh re-reads the milestone sentinel row; when `download_package_status = "ready"` is observed, the "Preparing..." indicator is replaced with a clickable **Download Now** link (the `download_package_url`). |
...
## Field summary — milestone sentinel row

These are the SP audit fields written by milestone-level button clicks. Per `SP_lists_authoritative.xlsx`, all live on a sentinel row in the Milestones list with `ItemNumber = "_milestone"` (or `0` per your choice, locked in `customizations/sharepoint_config/<deployment>.yaml` per FR-84).
...
1. **"Send Alerts for These Changes" = "Anything changes"** on every list HILDA reads from (Customers, Devices, Milestones, DeliveryItems). Without this, TPM direct field edits per FR-14 won't fire alerts.
2. **Subject line format**: `Alert_<ListName>_<Suffix> - <ItemTitle>` per FR-84 SP-alert convention. SP UI engineer's alert template MUST emit the 5 routing-key fields in the alert body (`customer_id`, `Model`, `ProjectID`, `MinorMilestone`, `ItemNumber`).
