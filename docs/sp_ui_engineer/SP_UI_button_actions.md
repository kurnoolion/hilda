...
**Milestone-level writes target the Milestones SP list row** per `[D-083]`. Milestone-level actions (Start Collection / Submit to Carrier / Close All Items / Refresh / Download Package) write the milestone-level column (e.g., `milestone_collection_started_at`) to the **single Milestones SP list row** for the milestone — one column write fires one SP-alert per FR-84 Source B routing. **No N-row cascade write; no `[D-082]` cascade-dedup needed** for milestone-button trigger writes. Per-DeliveryItem actions (Approve, Mark Closed, cell edits, etc.) continue to write to the individual `Deliverables_<customer_id>` row.
...
| **Enabled when** | `milestone.milestone_collection_started_at` is empty AND `milestone.status = "Not Started"` (`In Progress` removed 2026-06-20 — structurally impossible: status only flips to `In Progress` per FR-6 precedence ladder AFTER `milestone_collection_started_at` is set + items advance past `Not Started`; the original combo `milestone_collection_started_at IS NULL` + `status = "In Progress"` cannot occur) |
| **User prompt** | "Start collection for milestone `<name>`? This will send initial outreach to all R&D owners." (Yes / Cancel) |
| **What happens on click** | Set `milestone_collection_started_at = <now>` on the **single Milestones SP list row** for the milestone (one column write per `[D-083]`). |
| **What HILDA does next** | *(Background — out of your scope)* HILDA receives one SP-alert (Source B routing per FR-84), transitions each DI in `Deliverables_<customer_id>` from `Not Started` → `Open` via `[D-064]` REST writeback, creates PLM issues per (owner × milestone), fires email outreach to all owners, transitions each DI to `OutreachSent`, activates runtime polling channels (Email/NSD/PLM/CustomerJIRA per per-item `tracking_modality`), and sets `Milestone.status = "In Progress"` on the Milestones row. Item rows update via live polling. |
...
| **FR refs** | FR-8, FR-56 (e), FR-84 (Source B single-row write), `[D-083]` (2-list architecture) |
...
|---|---|
| **Where** | Milestone View top bar |
| **Enabled when** | All `milestone_gating = true` DeliveryItems in milestone are in `{ReadyForSubmission, SubmittedToCustomer}` AND at least one `milestone_gating = true` DI is in `ReadyForSubmission` (per FR-63 gating-aware lock 2026-06-17). **Items excluded from the enablement check** regardless of `milestone_gating`: (a) default work-item per FR-78 (`item_type = Default`); (b) items with `no_customer_upload = true` per FR-80; (c) `item_type = Confirmation` per FR-7 — all three have no carrier-submission path. **Also disabled when** any `milestone_gating = true` item is in `UnderPMReview` (FR-62 upload-induced revert blocker — PM must re-approve per FR-56 (e) before next Submit). Disabled-state hover tooltip: *"Submit blocked: `<N>` gating items in `UnderPMReview` — PM must re-approve per FR-56 (e). Items: `<list>`"* |
...
| **What happens on click** | Set `milestone_submission_triggered_at = <now>` on the **single Milestones SP list row** for the milestone (one column write per `[D-083]`). |
...
|---|---|
| **Where** | Milestone View top bar |
| **Enabled when** | All `milestone_gating=true` DIs are in `{SubmittedToCustomer, Closed}` AND at least one `milestone_gating=true` DI is in `SubmittedToCustomer`. **Non-gating items** (`milestone_gating=false`) may remain in any non-blocking state at click time. |
...
| **What happens on click** | **UPDATED 2026-06-20 per architect direction Option (b) — HILDA-owned per-item cascade; "atomic multi-row write" framing dropped (SP-2017 supports row-level atomicity only — no multi-row transactions)**. SP UI engineer's web part writes **ONLY the trigger field**: set `closed_all_items_triggered_at = <now>` on the **single Milestones SP list row** (one REST PATCH call → row-level atomic per SP-2017 → fires one SP-alert per FR-84 Source B). Per-item `delivery_state = Closed` writes on the N affected `Deliverables_<customer_id>` rows are NOT written by SP UI engineer's web part — those are owned by HILDA's `close_all_items` Celery task per FR-64 Option (b) lock 2026-06-20 (HILDA receives the Source B alert + iterates per-item writes via `[D-064]` REST writeback with retry semantics; default work-item row is NOT touched in either side of the cascade). |
| **What HILDA does next** | *(Background — out of your scope)* HILDA performs a two-part cascade per FR-64: (1) sets all `SubmittedToCustomer` items to `Closed`; (2) auto-advances any `milestone_gating=false` items in non-Closed states to `Closed` (action_type=`bulk_close_non_gating`). When all gating items + default work-item reach `Closed`, FR-76 `MilestoneStorageCleanup` fires — NSD `internal/<milestone>/` subtree is deleted permanently. Item rows update via live polling. |
...
| **What happens on click** | Set `refresh_requested_at = <now>` on the **single Milestones SP list row** for the milestone (one column write per `[D-083]`). |
...
| **What happens on click** | Per `[D-083]` single Milestones-row writes: (a) set `download_package_request_timestamp = <now>` on the **single Milestones SP list row** for the milestone (one column write); (b) immediately update SP UI to show "Package preparing..." indicator (replaces the button); (c) clear `download_package_url`, `download_package_status`, `download_package_generated_at` on the Milestones row (fresh assembly per click). |
| **What HILDA does next** | *(Background — out of your scope)* HILDA assembles the zip in background per FR-73 (individual files only per FR-77 Type-2 routing — no audit zips). On completion, HILDA writes back to the **single Milestones SP list row**: `download_package_url = https://hilda-proxy.corp/dl/<scoped_token>`, `download_package_status = "ready"`, `download_package_generated_at = <now>` via `[D-064]` REST. On failure: `download_package_status = "failed"` on Milestones row + `HildaOpsAlert` logged. |
...
- **YAML-only fields** (NOT TPM-editable): `milestone_gating` per `[D-078]`
...
## Field summary — milestone-level fields (live on the single Milestones SP list row per `[D-083]`)

These are the SP audit fields written by milestone-level button clicks. Per `[D-083]` 2-list architecture + `SP_lists_authoritative.xlsx`, all live as **columns on the single Milestones SP list row** for the milestone — one column write per button click fires one SP-alert per FR-84 Source B routing. No N-row cascade.
...
|---|---|---|---|
| Milestone banner styling (Not Started / In Progress / Completed / Delayed) | `Milestone.status` (read from the Milestones SP list row per `[D-083]`) | Banner color, label, status badge | FR-6 |
| Start Collection button visibility | `milestone_collection_started_at` (Milestones row) | Button rendered only when `milestone_collection_started_at IS NULL` | FR-6, FR-8 |
| Submit to Carrier button enablement | `delivery_state` + `milestone_gating` + `item_type` + `no_customer_upload` (all `Deliverables_<customer_id>` rows in milestone) | Button enabled when all `milestone_gating = true` DIs in `{ReadyForSubmission, SubmittedToCustomer}` AND ≥1 `milestone_gating = true` DI in `ReadyForSubmission` AND no gating DI in `UnderPMReview` (FR-62 revert blocker). Items excluded from check (regardless of gating): (a) default work-item (`item_type = Default`); (b) `no_customer_upload = true`; (c) `item_type = Confirmation` | FR-63 |
| Close All Items button enablement | `delivery_state` + `milestone_gating` (all Deliverables rows) | Enabled when all `milestone_gating=true` DIs in `{SubmittedToCustomer, Closed}` AND ≥1 `SubmittedToCustomer` | FR-64 |
| Milestone deadline display | `target_date` (Milestones row per `[D-085]`) | Banner subtitle + per-item deadline display (joined at row render time) | FR-11, FR-6 |
| Download Package status indicator | `download_package_status` + `download_package_url` (Milestones row) | Indicator state (preparing / ready / failed); Download Now link rendered when status = ready | FR-73 |
