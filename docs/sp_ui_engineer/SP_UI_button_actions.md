...
| Submit to Carrier button enablement | `delivery_state` + `is_milestone_gating` + `item_type` + `no_customer_upload` (all `Deliverables_<customer_id>` rows in milestone) | Button enabled when all `is_milestone_gating = true` DIs in `{ReadyForSubmission, SubmittedToCustomer}` AND ≥1 `is_milestone_gating = true` DI in `ReadyForSubmission` AND no gating DI in `UnderPMReview` (FR-62 revert blocker). Items excluded from check (regardless of gating): (a) default work-item (`item_type = Default`); (b) `no_customer_upload = true`; (c) `item_type = Confirmation` | FR-63 |
| Close All Items button enablement | `delivery_state` + `is_milestone_gating` (all Deliverables rows) | Enabled when all `is_milestone_gating=true` DIs in `{SubmittedToCustomer, Closed}` AND ≥1 `SubmittedToCustomer` | FR-64 |
