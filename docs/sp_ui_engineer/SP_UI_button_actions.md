...
| **What happens on click** | Two write groups in one SP transaction per `[D-083]`: **(a)** set `closed_all_items_triggered_at = <now>` on the **single Milestones SP list row** (the trigger write that fires one SP-alert); **(b)** atomically write per-item `delivery_state` transitions on the N affected `Deliverables_<customer_id>` rows (Part 1 gating items + Part 2 non-gating force-advance; default work-item row is NOT touched). |
