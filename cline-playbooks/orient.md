...
**Purpose**: load HILDA's project context. Run on the first conversation each session, OR
when context feels stale.

**Input**: optionally a module name (e.g., `sharepoint_integration`, `template_schema`).
...
4. Read `docs/compact/SYSTEM.md` — workload topology, K8s deployments, observability shape.
5. If the user named a module, read `core/src/<module>/MODULE.md`.
6. If `<env_dir>/state/hilda-mapping.json` exists, read it so you know the redaction table
...
- No prose explanation of "what HILDA is" — Teacher LLM already knows; the user already knows.
